...
CREATE PROCEDURE [dbo].[Create_ROI_Data_test] @processdate DATETIME
AS
		...
		UNION ALL
           ...
           FROM   red_dw.dbo.ds_sh_3e_timecard AS timecard_2
				  ...
				  INNER JOIN (   
                                 ...
                                 FROM   red_dw.dbo.ds_sh_3e_timekeeper
                                 WHERE  hrtitle IN ( 'Managing Partner','Partner','Senior Partner'
 )
														   
														   ) AS refer ON timecard_2.timekeeper = refer.timekeeper
