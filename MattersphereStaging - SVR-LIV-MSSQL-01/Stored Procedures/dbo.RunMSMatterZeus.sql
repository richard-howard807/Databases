...
CREATE PROCEDURE [dbo].[RunMSMatterZeus]
(
	 @ID INT, 
	 @clNo NVARCHAR (12),
	 @fileNo NVARCHAR (20) ,
...
--Validate the partner exists. mapc
...
-- MAPC Populate ComboBoxes
	DECLARE @CategoryID NVARCHAR(50) = (SELECT COALESCE(matterCategory,NULL) FROM [MS_PROD].[dbo].[udMatterCategory] WHERE matterType = @fileType AND active = 1)
...
BEGIN
	...
	INSERT INTO  MS_PROD.[config].[dbFile]
		...
		@filePrincipleMSID,@fileDept,@fileType,
		@fileFundCode,@fileCurISoCode,@fileStatus,
...
GO
GRANT EXECUTE ON  [dbo].[RunMSMatterZeus] TO [lnksvrdatareader_sql02]
GO
GRANT EXECUTE ON  [dbo].[RunMSMatterZeus] TO [ZurichImportServiceSQL]
GO
