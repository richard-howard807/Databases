...
CREATE PROCEDURE [dbo].[RunMSMatterMAPC]
(
	 @ID INT, 
	 @clNo NVARCHAR (12),
	 @fileNo NVARCHAR (20) ,
...
--- Get FILEID
	...
	SET @FileID=(SELECT fileID FROM MSFile WITH (NOLOCK) WHERE clID=@clID AND fileNo=RTRIM(CAST(@MatterNo AS VARCHAR)))
     ...
     ([fileID],[cboPracticeArea],[cboTeam],[cboBusinessLine],[cboDepartment],cboPartner,FEDCode)
	 SELECT @FileID, @fileSectionGroup, @fileSection, @BusinessLine, @fileDept,@PartnerMSID,@FEDCode
