...
-- Date:		20160812
-- Description:	Listing of all transactions on the escrow
--========================================================
Version 
*/
-- exec [converge].[derwent_cashbook_build]  '20170131'

CREATE PROCEDURE [converge].[derwent_cashbook_build] 
(
...
AS 
	...
	IF EXISTS (SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#DerwentCashbook')) 
				DROP TABLE #DerwentCashbook
	...
	INTO #DerwentCashbook			
	FROM [converge].[vw_replicated_trust_balance] tb
	WHERE tb.gl_date	BETWEEN '20111001' AND @EndDate  
	AND (BankAcctIndex  = '96')   


 /*
	The code below manually inserts two transactions that are missing from the trust balance
*/
	
	INSERT INTO #DerwentCashbook
	        ( client_matter_number ,
           legacy_client_matter_number ,
           client ,
           matter ,
	...
	client_matter_number ,
           ...
           bank_account_number ,
           amount ,
          'manual' legacy_sequence_number ,
          'manual' sequence_number ,
           narrative ,
           transaction_type_code ,
           transaction_type_description ,
          '20161020' post_date ,
        ...
        '20161020'   tran_date ,
         'R'  recon_status ,
        '600051'    document_number ,
           postsource ,
           IsReversed ,
           IsDeposit ,
           --TrustCheck_GLDate ,
           Exclude 
         -- '20161020'  checkdatenull
   

	FROM #DerwentCashbook
	WHERE sequence_number IN ('3e1103687')
	...
	client_matter_number ,
        ...
        '20161028'   tran_date ,
         'R'  recon_status ,
        '600052'    document_number ,
           postsource ,
           IsReversed ,
           IsDeposit ,
           --TrustCheck_GLDate ,
           Exclude 
          --'20161028'  checkdatenull
   

	FROM #DerwentCashbook
	WHERE sequence_number IN ('3e1104072')
	...
	TRUNCATE TABLE [Reporting].[converge].[cashbook_st_derwent]
		
	INSERT INTO [Reporting].[converge].[cashbook_st_derwent]
    
    SELECT 
			
				CASE 
				 WHEN IsReversed = 0 AND IsDeposit = 0 AND  transaction_type_code = 'EFT' THEN 'PAYMENT'
				 WHEN transaction_type_code IN ('CLCTR','GEN','EFT') THEN 'TRANSFER' -- client transfer
		...
		,	legacy_sequence_number
		,   cafintrn.gl_date
		,   cafintrn.tran_date [transaction_date]
		,	GETDATE() AS InsertDate

		
		  
	                      
    FROM    #DerwentCashbook AS cafintrn 
            ...
            LEFT OUTER JOIN ( SELECT    VE00167.case_id,
					...
					OUTER APPLY (	SELECT SUM(VE00160.case_value) NewTotalPaid_Total
									...
									LEFT JOIN #DerwentCashbook AS caftotal 
