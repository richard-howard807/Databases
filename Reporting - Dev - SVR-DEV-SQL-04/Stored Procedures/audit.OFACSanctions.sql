...
,CASE WHEN DateClosed IS NULL AND COALESCE(AUD214.case_date,AUD214_b.case_date,dteDateSanRev ) IS NULL OR CONVERT(Date,[Last Updated],103) > CONVERT(Date,COALESCE(AUD214.case_date,AUD214_b.case_date,dteDateSanRev ),103) THEN 1 ELSE 0 END [Re-Check Needed]
...
LEFT OUTER JOIN (SELECT mg_client,mg_matter,mg_datcls, 1 AS SourceID FROM axxia01.dbo.camatgrp  WITH (NOLOCK)  ) AS FEDClosures
