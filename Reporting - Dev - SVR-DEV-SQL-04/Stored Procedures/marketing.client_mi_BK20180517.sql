...
CREATE PROCEDURE [marketing].[client_mi_BK20180517]
(
...
*/
	...
	SELECT	
		...
		,SUM(CASE WHEN bill_date.calendar_date BETWEEN @DateFrom AND @DateTo THEN ISNULL([minutes_recorded],0) ELSE 0 END ) [billed_time]
	...
	LEFT JOIN (SELECT	minutes_recorded.dim_bill_key
						,minutes_recorded.dim_client_key
						,minutes_recorded.dim_bill_date_key
						,fee_earner.fed_code
				...
				GROUP BY  
						minutes_recorded.dim_bill_key
						,minutes_recorded.dim_client_key
						,minutes_recorded.dim_bill_date_key
						,fee_earner.fed_code
				) billed_time ON billed_time.dim_bill_date_key = finance.dim_bill_date_key
				AND billed_time.dim_bill_key = finance.dim_bill_key
				AND billed_time.dim_client_key = finance.dim_client_key
				AND billed_time.fed_code = finance.fed_code_fee_earner	
	...
	--10 MINUTES 30 SECONDS  --  unacceptable
