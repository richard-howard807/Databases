...
CREATE PROCEDURE [marketing].[client_mi_BK20180416]
(
...
(
	SELECT	
		...
		fee_earner.hierarchylevel4 [team]
	...
	INNER JOIN red_dw.dbo.dim_fed_hierarchy_history fee_earner ON finance.fed_code_fee_earner = fee_earner.fed_code AND fee_earner.dss_current_flag = 'Y'
	WHERE bill_date.calendar_date BETWEEN @PreviousDateFrom AND @PreviousDateTo
	GROUP BY
		fee_earner.hierarchylevel4 
		--,@fin_month_display
)
	...
	SELECT	
		client_group_name
		,fee_earner.hierarchylevel2 [division]
		,fee_earner.hierarchylevel3 [department]
		,fee_earner.hierarchylevel4 [team]
		,ISNULL(pc_previous.previous_bill_amount,0) previous_bill_amount
		,bill_date.fin_month_display
	...
	GROUP BY
		 client_group_name
		,fee_earner.hierarchylevel2 
		,fee_earner.hierarchylevel3 
		,fee_earner.hierarchylevel4 
