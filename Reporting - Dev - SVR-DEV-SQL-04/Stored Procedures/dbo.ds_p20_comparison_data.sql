...
CREATE   PROCEDURE [dbo].[ds_p20_comparison_data]
...
BEGIN
	...
	IF OBJECT_ID('tempdb..#damages_paid') IS NOT NULL DROP TABLE #damages_paid;
	...
	--WHERE 
		--IQ1.fee_arrangement = @fee_arrangement
		--AND IQ1.proceedings_issued = @proceedings_issued
		--AND IQ1.suspicion_of_fraud = @suspicion_of_fraud
		--AND IQ1.department = @department
		--AND IQ1.bin = @bin
		--AND IQ1.work_type_group = @work_type_group
		--AND IQ1.team = @team 
	;

	SELECT 
		* 
		--,CASE WHEN matter_owner_full_name = @target_matter_owner THEN 0.0 ELSE 1.0 END AS X
	FROM #damages_paid;

	--SELECT 
	--	matter_owner_full_name
	--	,PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY value_for_target) OVER (PARTITION BY matter_owner_full_name) AS median
	--FROM #damages_paid
	--;
