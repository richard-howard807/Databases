...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[AllDebtDumpOtherPayor] 
AS 
BEGIN
--Debt Dump - Other Payor
                          
SELECT 
...
WHERE 
--InvPayor.IsDefault=0
(Payor.DisplayName<>dim_client.client_name collate database_default
AND NOT (Payor.DisplayName  LIKE '%Ageas%' AND dim_client.client_name  LIKE '%Ageas%')
AND NOT (Payor.DisplayName  LIKE '%Zurich%' AND dim_client.client_name  LIKE '%Zurich%'))
--AND InvPayor.BalAmt>0)
--OR (InvPayor.BalTax>0 
AND 
(InvPayor.BalBOA>0
OR InvPayor.BalFee>0
OR InvPayor.BalHCo>0
OR InvPayor.BalSCo>0
OR InvPayor.BalInt>0
OR InvPayor.BalOth>0)
