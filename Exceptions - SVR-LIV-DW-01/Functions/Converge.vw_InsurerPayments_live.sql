...
-- SELECT * FROM [Converge].[vw_InsurerPayments](334527, '2012-04-16')
-- SELECT * FROM [Converge].[vw_InsurerPayments_live](334527)

CREATE FUNCTION [Converge].[vw_InsurerPayments_live] (
	  @CaseID int
)
RETURNS @rettab TABLE (case_id int, TotalInsurerPaidIncFees money, TotalInsurerPaidExcFees money)
AS
BEGIN
	INSERT INTO @rettab
	SELECT cashdr.case_id
		 , SUM(ISNULL(childval.case_value,0)) AS TotalInsurerPaidIncFees
		 , SUM(CASE WHEN LOWER(parentlup.sd_listxt) LIKE '%loss adjuster fee%'
			   THEN 0 
			   ELSE ISNULL(childval.case_value,0) 
			   END) AS TotalInsurerPaidExcFees
