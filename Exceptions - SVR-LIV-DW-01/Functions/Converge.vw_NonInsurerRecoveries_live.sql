...
-- SELECT * FROM [Converge].[vw_NonInsurerRecoveries](339404, '2012-05-01')
-- SELECT * FROM [Converge].[vw_NonInsurerRecoveries_live](339404)

CREATE FUNCTION [Converge].[vw_NonInsurerRecoveries_live] (
	  @CaseID int
)
RETURNS @rettab TABLE (case_id int, TotalRecovered money)
AS
BEGIN
	INSERT INTO @rettab
	SELECT cashdr.case_id
		 , SUM(ISNULL(childval.case_value,0)) AS TotalRecovered
