...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[AllMotorRecoveries]
AS
BEGIN

SELECT  
CASE WHEN fact_dimension_main.client_code='00752920' AND UPPER(matter_description) LIKE 'ELITE%' THEN 'Elite'
WHEN fact_dimension_main.client_code='00752920' AND UPPER(matter_description) LIKE 'TRADEX%' THEN 'Tradex'
WHEN fact_dimension_main.client_code='00752920' THEN  'Armour' ELSE client_name END AS Client,
...
LEFT OUTER JOIN (SELECT a.client_code,a.matter_number,SUM(wiphrs) AS HrsWorked FROM red_dw.dbo.fact_all_time_activity AS a
 INNER JOIN (
 SELECT dim_matter_header_current.client_code,dim_matter_header_current.matter_number FROM red_dw.dbo.dim_matter_header_current
 INNER JOIN red_dw.dbo.dim_detail_core_details
  ON dim_detail_core_details.client_code = dim_matter_header_current.client_code
  AND dim_detail_core_details.matter_number = dim_matter_header_current.matter_number
WHERE referral_reason='Recovery'
) AS b
ON b.client_code = a.client_code
AND b.matter_number = a.matter_number
GROUP BY a.client_code,a.matter_number) AS HrsWorked
