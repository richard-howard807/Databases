...
,udMIAuditNHSSearchList.cboCorrectCos

, [Correct costs scheme?] = cboCorrectCos.CDDesc
...
--,[Correct costs scheme?] = 
,dim_child_detail.[nhs_correct_costs_scheme] -- MT 20220902

---------------SCORING ---------------------------------



, CASE WHEN ISNULL(cboCorrectCos.CDDesc, '') IN ('Not applicable', 'N/A', '') THEN 0 ELSE 1 END                                          --1  [Correct costs scheme?] also known as Correctly billed?]
+ CASE WHEN panel_steps_to_avoid_litigation IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                        --2  [Did Panel take steps to avoid Litigation?]
+ CASE WHEN nhs_proactivity_on_file IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                                --3  [Proactivity on File?]
+ CASE WHEN nhs_correct_costs_scheme IN ('Not applicable', 'N/A') THEN 0 ELSE 1 END                                --4  Costs Correctly Billed 	
+ CASE WHEN nhs_damages_reserve_accurate IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                           --5 [Damages reserve accurate?] --Damages reserve - change deadline met?
+ CASE WHEN nhs_c_costs_reserve_accurate IN ('Not applicable', 'N/A') THEN 0 ELSE 1 END                            --6 [C Costs reserve accurate?]
+ CASE WHEN nhs_d_costs_reserve_accurate IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                           --7  [D Costs reserve accurate?]
+ CASE WHEN nhs_probabilty_reserve_accurate IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                        --8 [Probabilty reserve accurate?] --Probability change deadline met?
+ CASE WHEN nhs_esd_reserve_accurate IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                               --9   [ESD reserve accurate?] --ESD change deadline met?
+ CASE WHEN nhs_breach_of_duty_decision_correct IN ('Not applicable', 'N/A') THEN 0 ELSE 1 END                     --10  [Breach of duty decision correct?]
+ CASE WHEN nhs_causation_decision_correct IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                         --11  [Causation decision correct?]
+ CASE WHEN nhs_correct_choice_of_expert IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                           --12  [Correct choice of expert?]
+ CASE WHEN nhs_supervisory_process_followed IN ('Not applicable', 'N/A') THEN 0 ELSE 1 END                        --13 [Supervisory process followed?]
+ CASE WHEN nhs_sla_instructions_ack_in_48_hours IN ('Not applicable', 'N/A') THEN 0 ELSE 1 END                    --14 [SLA: Instructions ack in 48 hours?]
+ CASE WHEN nhs_sla_first_report_deadline_met IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                      --15  [SLA: First report deadline met?]
+ CASE WHEN nhs_sla_follow_up_report_deadlines_met IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                 --16 [SLA: Follow up report deadlines met?]
+ CASE WHEN nhs_sla_pre_trial_report_deadline_met IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                  --17 [SLA: Pre-trial report deadline met?]
+ CASE WHEN nhs_sla_advice_on_claimant_p36_offer_deadline_met IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END      --18 [SLA: Advice on claimant P36 offer deadline met?]
+ CASE WHEN nhs_advice_contains_all_required_fields_quality IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END        --19 [Advice contains all required fields/quality?]
+ CASE WHEN covid_direct_reported IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                                  --20  [Is the case a covid direct case and reported as such?]
+ CASE WHEN covid_indirect_reported  IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                               --21  [Does the case have covid indirect elements and reported as such?]
+ CASE WHEN nhs_court_deadlines_met IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END                                --22  [Court deadlines met?]
...
+ CASE WHEN nhs_internal_and_nhsr_cms_consistent_and_accurate IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END      --25 [Internal and NHSR CMS consistent and accurate?]
+ CASE WHEN fact_child_detail.nhs_hard_leakage_quantified_at >= 0 THEN 1 ELSE 0 END                                --26 [Hard Leakage quantified at £] --20
+ CASE WHEN nhs_soft_leakage_difficult_unable_to_quantify IN ('Not applicable', 'N/A')  THEN 0 ELSE 1 END          --27 [Soft Leakage -difficult/unable to quantify]
...
CASE WHEN cboCorrectCos.CDDesc IN ('Not applicable','No', 'N/A') THEN 0 
WHEN cboCorrectCos.CDDesc IS NULL THEN 0
WHEN cboCorrectCos.CDDesc ='Partial' THEN 0.5
ELSE 1 END --1
...
ELSE 1 END --19
+ CASE WHEN dim_child_detail.covid_direct_reported IN ('Not applicable','No', 'N/A') THEN 0 
 WHEN covid_direct_reported ='Partial' THEN 0.5
 WHEN covid_direct_reported = 'Yes and not reported' THEN 0
ELSE 1 END --20 
+ CASE WHEN covid_indirect_reported IN ('Not applicable','No', 'N/A') THEN 0 
 WHEN covid_indirect_reported ='Partial' THEN 0.5
 WHEN covid_indirect_reported = 'Yes and not reported' THEN 0
ELSE 1 END --21
...
ELSE 1 END --25
+ CASE WHEN fact_child_detail.nhs_hard_leakage_quantified_at <>0 THEN 0 ELSE 1 END --26
+ CASE WHEN nhs_soft_leakage_difficult_unable_to_quantify  IN ('Not applicable','No', 'N/A') THEN 1 
WHEN nhs_soft_leakage_difficult_unable_to_quantify ='Partial' THEN 0.5 ELSE 0 END --27
...
,[Naming conventions used?] = MAX(cboNameConvUsed.cdDesc) OVER(PARTITION BY ms_fileid)	
,[Misfiled / inappropriate content?] = MAX(cboMisfiledInap.cdDesc) OVER(PARTITION BY ms_fileid)	
...
WHERE txtMSCode = 'cboNameConvUsed' AND txtMSTable = 'udMIAuditNHSSearchList'
)
 cboNameConvUsed ON cboNameConvUsed.cdCode = [udMIAuditNHSSearchList].cboNameConvUsed
...
/*udMIAuditNHSSearchList.cboCorrectCos -- Correct Costs Scheme*/
LEFT JOIN (SELECT DISTINCT cdCode, cdDesc  FROM  MS_PROD.dbo.udMapDetail
JOIN ms_prod.dbo.dbCodeLookup ON txtLookupCode = cdType
WHERE txtMSCode = 'cboCorrectCos' AND txtMSTable = 'udMIAuditNHSSearchList'
)
 cboCorrectCos ON cboCorrectCos.cdCode = [udMIAuditNHSSearchList].cboCorrectCos
