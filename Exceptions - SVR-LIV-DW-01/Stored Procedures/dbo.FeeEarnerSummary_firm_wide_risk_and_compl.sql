...
create PROCEDURE [dbo].[FeeEarnerSummary_firm_wide_risk_and_compl]
(
    @client_code NVARCHAR(8), 
	@matter_number NVARCHAR(8),
	@DatasetID NVARCHAR(max)=N'226'
)
AS
BEGIN
...
AND ISNULL(LOWER(exclude_from_exceptions_reports),'') <> 'yes'
GROUP BY dim_fed_hierarchy_history.employeeid,date_closed_case_management
    ...
    (
        ...
        WHERE df.datasetid = ex.datasetid
             ...
             AND ex.client_code = @client_code
			 AND ex.matter_number = @matter_number
    ...
    WHERE (
         ...
         AND feu.client_code = @client_code
			 AND feu.matter_number = @matter_number
