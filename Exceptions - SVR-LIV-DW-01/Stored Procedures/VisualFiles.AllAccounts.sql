...
--EXEC [VisualFiles].[AllAccounts] 'Equidebt','2012-01-01','2013-02-28'





CREATE PROCEDURE [VisualFiles].[AllAccounts]
    (
...
ON AccountInfo.mt_int_code = LastPayment.mt_int_code                
    WHERE   
    --AccountInfo.[mt_int_code] NOT IN (
    --        SELECT  mt_int_code
    --        FROM    VFile_Streamlined.dbo.AccountInformation
    --        WHERE   ISNULL(CAS_BatchNumber,'') LIKE '%LBA%'
    --                AND ClientName = 'Equidebt' )
    --                  AND 
                      AccountInfo.DateOpened BETWEEN @StartDate
                                       AND     @EndDate
ORDER BY AccountInfo.DateOpened
END
ELSE
BEGIN
    SELECT  CDE02ClientAccountNo AS ClientAccountNo ,
...
ON AccountInfo.mt_int_code = LastPayment.mt_int_code                
    WHERE  
     ...
     --               AND ClientName = 'Equidebt' )
            --AND 
            AccountInfo.ClientName = @ClientName
            AND AccountInfo.DateOpened BETWEEN @StartDate
                                       AND     @EndDate
 ORDER BY AccountInfo.DateOpened
    END 
