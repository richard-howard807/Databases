...
CREATE PROCEDURE [dbo].[ZurichDiseaseNewListings2019_backup20191106]
 ...
 --   LEFT JOIN  red_dw.dbo.dim_parent_detail parent ON parent.client_code = fact_dimension_main.client_code AND parent.matter_number = fact_dimension_main.matter_number
    ...
    (
        ...
        (
		 ...
		 /*--Defence Costs Resesrve-- */
		  LEFT OUTER JOIN
            (
                SELECT client_code,
                       matter_number,
                       dim_parent_key,
                       nihl_defence_costs_reserve_net [Defence Cost Reserve] -- ISNULL(child.nihl_damages_reserve_net,0) + ISNULL(child.nihl_claimants_costs_reserve_net,0) [Claimants Cost Reserve ]
					  
                FROM  red_dw.dbo.fact_child_detail  
				WHERE  nihl_defence_costs_reserve_net IS NOT NULL
            ) AS dc_reserves
                ON Parent.dim_parent_key = dc_reserves.dim_parent_key

			 /*--Claimants Costs Resesrve-- */
		  LEFT OUTER JOIN
            (
                SELECT client_code,
                       matter_number,
                       dim_parent_key,
                       ISNULL(nihl_damages_reserve_net,0) + ISNULL(nihl_claimants_costs_reserve_net,0) [Claimants Cost Reserve]
					  
                FROM  red_dw.dbo.fact_child_detail  
				WHERE  COALESCE(nihl_damages_reserve_net,nihl_claimants_costs_reserve_net) IS NOT NULL
            ) AS cc_reserves
                ON Parent.dim_parent_key = cc_reserves.dim_parent_key
            LEFT OUTER JOIN
            (
                SELECT client_code,
                       matter_number,
