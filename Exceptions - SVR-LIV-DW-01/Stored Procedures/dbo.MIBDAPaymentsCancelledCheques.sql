...
SET ANSI_NULLS ON
GO
--- 2020-02-10 changed to only look at Claims HF 

CREATE PROCEDURE [dbo].[MIBDAPaymentsCancelledCheques] --EXEC [dbo].[MIBDAPayments] '2018-11-01','2018-11-01'
(
@StartDate AS DATE
,@EndDate AS DATE
)
...
,AllPayments.hierarchylevel2hist
...
END) AS PaymentType 
,red_dw.[dbo].[datetimelocal] (dteDAPayment) PaymentDate
...
AND red_dw.[dbo].[datetimelocal] (dteApproved) BETWEEN @StartDate AND @EndDate

) AS AllData
) AllPayments 
WHERE [Adjustment Y/N]='Y'
AND AllPayments.hierarchylevel2hist = 'Legal Ops - Claims'
...
--WHERE MIB045='Yes'
--AND DateApproved BETWEEN @StartDate AND @EndDate
--) AS AllData
--) AllPayments 
--WHERE [Adjustment Y/N]='Y'
END
