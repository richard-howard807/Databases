...
--============================================
-- Author: Lucy Dickinson (mostly ... okay only a bit)
-- Date:	20160818
-- Description:  This is the Codeve cashbook, the cashbooks have been seperated out in order to deal with a few issues
--					once July is out of the way we can try and consolidate them all again.
--============================================
CREATE PROCEDURE [converge].[cashbook_codeve]
(
    @EndDate AS DATETIME
...
AS 
	...
	--DECLARE @EndDate datetime = '20161003'
        
	 
	IF EXISTS (SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#FTBCodeveCashbook')) 
				DROP TABLE #FTBCodeveCashbook
  ...
  --SELECT * FROM #FTBCodeveCashbook
    ...
    SELECT  DISTINCT
            ...
            ISNULL((CASE WHEN COALESCE(payments.PaymentNumber, Recoveries.RecoveryNumber, '0') <> cafintrn.sequence_number COLLATE DATABASE_DEFAULT THEN NULL
						 WHEN PaymentGross IS NOT NULL AND cafintrn.trust_transaction_type IN ( 'CLPFC', 'CLCRC','AUTHC','CHQ' ) THEN PaymentGross
				  WHEN RecoveryAmount IS NOT NULL AND cafintrn.trust_transaction_type IN ( 'CLRTC' ) THEN RecoveryAmount
            ...
            ISNULL((CASE WHEN COALESCE(payments.PaymentNumber , Recoveries.RecoveryNumber, '0') <> cafintrn.sequence_number COLLATE DATABASE_DEFAULT THEN NULL
				  WHEN PaymentNet IS NOT NULL AND cafintrn.trust_transaction_type IN ( 'CLPFC', 'CLCRC','AUTHC','CHQ' ) THEN PaymentNet
				  WHEN RecoveryAmount IS NOT NULL AND cafintrn.trust_transaction_type IN ( 'CLRTC' ) THEN RecoveryAmount
				  ELSE NULL
			 END),0) AS PaymentNet,
		   ...
		   CASE 
                 ...
                 WHEN trust_transaction_type = 'CLCDC' THEN 'TOPUP' -- Direct credit client account
				 WHEN cafintrn.recon_status <> 'R' THEN 'UNPRESENTED'
				 WHEN trust_transaction_type IN ( 'CLPFC', 'CLCRC','AUTHC','AUTHE' ) THEN 'PAYMENT' --Client payment from client, returned cheque client account
    ...
    FROM    #FTBCodeveCashbook AS cafintrn WITH ( NOLOCK )
		    LEFT OUTER JOIN axxia01.dbo.cashdr AS cashdr WITH  ( NOLOCK ) 
							ON	cafintrn.legacy_client_matter_number = RTRIM(cashdr.client) + '-' + cashdr.matter  COLLATE DATABASE_DEFAULT 
            ...
            LEFT OUTER JOIN ( SELECT    VE00167.case_id,
                              ...
                              --WHERE ISNUMERIC(VE00328.case_text) = 1
                                               ...
                                               CASE WHEN trust_transaction_type IN ('CLCRC','CHQ')  THEN Recoveries.RecoveryAmount * -1 
