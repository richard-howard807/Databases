...
dim_matter_header_current.billing_arrangement [Rate Arrangement],
      ...
      dim_matter_worktype.[work_type_code] AS [Work Type Code],
	   ...
	   /*Flags - Background fills*/
[instruction_type_flag] = CASE WHEN TRIM(LOWER(dim_instruction_type.instruction_type)) = 'unknown' THEN 'Red' ELSE  'Transparent' END,
[Delegated_Flag] =   
 CASE WHEN   LOWER(dim_matter_header_current.delegated )=  'y'  AND TRIM(dim_detail_core_details.[aig_current_fee_scale]) = 'AFA - Non-Delegated Authority ("D/A")' THEN 'Red'
	  WHEN LOWER(dim_matter_header_current.delegated )=  'n'  AND TRIM(dim_detail_core_details.[aig_current_fee_scale]) = 'AFA - Delegated Authority ("D/A")' THEN 'Red'
      WHEN LOWER(dim_matter_header_current.delegated ) NOT IN ('n' ,'y' ) THEN 'Red'
	  ELSE 'Transparent' END ,

[CurrentFeeScaleFlag] = 
 CASE WHEN   LOWER(dim_matter_header_current.delegated )=  'y'  AND TRIM(dim_detail_core_details.[aig_current_fee_scale]) = 'AFA - Non-Delegated Authority ("D/A")' THEN 'Red'
	  WHEN LOWER(dim_matter_header_current.delegated )=  'n'  AND TRIM(dim_detail_core_details.[aig_current_fee_scale]) = 'AFA - Delegated Authority ("D/A")' THEN 'Red'
	  WHEN TRIM(dim_detail_core_details.[aig_current_fee_scale]) IN ('AFA D/A - Hourly', 'AFA Non-D/A - Hourly') THEN 'Red'
	  ELSE 'Transparent' END ,



[ClientsClaimHandler_Flag] = CASE WHEN dim_detail_core_details.[clients_claims_handler_surname_forename] IS NULL THEN 'Red'
                                  WHEN ISNULL(dim_detail_core_details.[clients_claims_handler_surname_forename], '') NOT LIKE '%,%' THEN 'Red' ELSE 'Transparent' END,


[RateArrangementFlag] = CASE WHEN TRIM(dim_matter_header_current.billing_arrangement) =  'HRLY' THEN 'Red' ELSE 'Transparent' END,


[SLAFileOpening_Flag] =  CASE WHEN [dbo].[ReturnElapsedDaysExcludingBankHolidays](date_instructions_received, red_dw.dbo.dim_matter_header_current.date_opened_case_management) <0  THEN 'Yellow'
                              WHEN [dbo].[ReturnElapsedDaysExcludingBankHolidays](date_instructions_received, red_dw.dbo.dim_matter_header_current.date_opened_case_management) > 2 THEN 'Yellow'  
							  ELSE 'Transparent' END,

[Date initial Acknowledgement to Claims Handler] = dim_detail_core_details.[aig_grp_date_initial_acknowledgement_to_claims_handler],

[Date initial Acknowledgement to Claims Handler_Flag] = CASE WHEN ISNULL(dim_detail_core_details.[aig_grp_date_initial_acknowledgement_to_claims_handler], '') = '' THEN 'Red' ELSE 'Transparent' END,

[SLA Acknowledgement] = [dbo].[ReturnElapsedDaysExcludingBankHolidays](date_instructions_received, dim_detail_core_details.[aig_grp_date_initial_acknowledgement_to_claims_handler]),
