...
create PROCEDURE [dbo].[MI_Exception_Summary_firm_wide_OK_TEST]
(
...
SELECT dim_fed_hierarchy_history.employeeid,
		...
		, CASE WHEN dim_matter_header_current.date_closed_case_management IS NULL THEN 1 ELSE 0 END AS Openexceptions
		, CASE WHEN dim_matter_header_current.date_closed_case_management IS NOT NULL THEN 1 ELSE 0 END AS closedexcptions
       ...
       SUM(   CASE
              ...
              END
                         ...
                         ELSE
                             0
                     END
                 ) total_cases, cases_closed.Openexceptions, cases_closed.closedexcptions
...
FROM Exceptions.dbo.MI_Management_firm_wide
    ...
    (
        SELECT dfhh.employeeid,
				 CASE WHEN dmh.date_closed_case_management IS NULL THEN 1 ELSE 0 END AS Openexceptions,
		CASE WHEN dmh.date_closed_case_management IS NOT NULL THEN 1 ELSE 0 END AS closedexcptions, 
               COUNT(dmh.case_id) cases
        FROM red_dw.dbo.fact_dimension_main fdm
            INNER JOIN red_dw.dbo.dim_matter_header_current dmh WITH (NOLOCK)
                ON dmh.dim_matter_header_curr_key = fdm.dim_matter_header_curr_key
...
FROM #results
    LEFT JOIN (
				...
				#critria_cases.Openexceptions, 
				#critria_cases.closedexcptions
				
				FROM #critria_cases
				GROUP BY open_closed , #critria_cases.Openexceptions, #critria_cases.closedexcptions,
				employeeid 
				) result GROUP BY 
