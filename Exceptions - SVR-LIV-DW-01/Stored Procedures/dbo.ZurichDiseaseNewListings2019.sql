...
-- ES 2022-09-14 - #167123, added hours recorded and hours billed
-- =============================================
...
from red_dw.dbo.fact_dimension_main
 ...
 --   LEFT JOIN  red_dw.dbo.dim_parent_detail parent ON parent.client_code = fact_dimension_main.client_code AND parent.matter_number = fact_dimension_main.matter_number
	...
	LEFT OUTER JOIN (SELECT fact_billable_time_activity.dim_matter_header_curr_key, SUM(fact_billable_time_activity.minutes_recorded)/60 AS hours_recorded 
			...
			GROUP BY fact_billable_time_activity.dim_matter_header_curr_key) AS recorded 
	ON recorded.dim_matter_header_curr_key = fact_dimension_main.dim_matter_header_curr_key

	LEFT OUTER JOIN (SELECT fact_bill_billed_time_activity.dim_matter_header_curr_key, SUM(fact_bill_billed_time_activity.minutes_recorded)/60 AS hours_billed
			...
			AND dim_matter_header_current.master_client_code='Z1001'
			GROUP BY fact_bill_billed_time_activity.dim_matter_header_curr_key) AS billed
	ON billed.dim_matter_header_curr_key = fact_dimension_main.dim_matter_header_curr_key
