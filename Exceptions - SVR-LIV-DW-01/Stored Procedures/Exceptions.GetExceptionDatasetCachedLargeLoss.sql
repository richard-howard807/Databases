...
--[Exceptions].[GetExceptionDatasetCachedLargeLoss] 'Large Loss - Mandatory Fields,Large Loss - Within 14 Days,Large Loss - Within 12 Months','All','ALL',0


CREATE PROCEDURE [Exceptions].[GetExceptionDatasetCachedLargeLoss] (
		@DatasetName VARCHAR(255)
	  , @ClientGroup VARCHAR(10)
	, @Debug BIT = 1 -- Returns the sql statement without executing it
	)
AS
BEGIN

--Test purpose
--DECLARE @DatasetName VARCHAR(255) = 'Large Loss - Mandatory Fields,Large Loss - Within 14 Days,Large Loss - Within 12 Months'
--DECLARE @Debug BIT = 1
--DECLARE @FeeEarnerCode VARCHAR(MAX) = 'AAZ,AGL,AHM,AKR1,ALN,AMY,AOC2,AWR,
--														BHD,BWM,CCL1,CHM,CLM,DAH,DCT,DJC,DJN,DKE,DSC,DWL,EDX,EJW,EMC,EMC1,EPX,FEA,HCK,HMM,JBI,JCP,JNG,JTW6,KMJ,KSM,LCN,
--														LUE,MPO,MSS,MSY,NAD,NDN,NRC,NSX,NTH,OGM,PAS,PAT,PAU,PPF,PRP,RBZ,RLW,RMC,RMS,SDP,SDP1,SEH1,SJW,SHO,SVS,TBS,TSM,TSX,TUD2,VEC,WAQ'
--DECLARE @ClientGroup VARCHAR(10) = 'ALL';
	...
	DECLARE @fed_codes nvarchar(max)

SELECT 
   @fed_codes = STUFF( (SELECT distinct ',' + fed_code 
                             FROM red_Dw.dbo.fact_exceptions_update 
							 WHERE datasetid IN (42,43,44)
                             --ORDER BY fed_code
                             FOR XML PATH('')), 
                            1, 1, '') 
	...
	/*Create FeeEarnerSummary */
	--drop table #FeeEarnerSummary
	SET @Sql = @Sql + 'CREATE TABLE #FeeEarnerSummary (case_id INT,exstring NVARCHAR(MAX),tabstring NVARCHAR(MAX),tab_name NVARCHAR(MAX),tab_order int,MIProjectExceptionTotal int,MIProjectCriticalTotal int)' + CHAR(10) + CHAR(10)
	
	SET @Sql = @Sql + 'INSERT  INTO #FeeEarnerSummary (case_id,exstring,tabstring,tab_name,tab_order,MIProjectExceptionTotal, MIProjectCriticalTotal)' + CHAR(10) + CHAR(10)
	SET @Sql = @Sql + 'exec [Exceptions].[FeeEarnerSummaryLargeLoss]'''+ @DatasetID + ''',''' + @fed_codes + ''',''ALL''' + CHAR(10) + CHAR(10)
	...
	SET @Sql = @Sql + 'FROM (SELECT * FROM red_dw.dbo.ds_sh_exceptions_values WHERE (ds_sh_exceptions_values.String IS NOT NULL OR ds_sh_exceptions_values.Number IS NOT NULL OR ds_sh_exceptions_values.Date IS NOT NULL ) AND ds_sh_exceptions_values.Flag IS NULL ' + CHAR(10)
	SET @Sql = @Sql + 'UNION ALL' +CHAR(10)
	SET @Sql = @Sql + 'SELECT * FROM red_dw.dbo.ds_sh_exceptions_values WHERE ds_sh_exceptions_values.Flag IS NOT NULL)a ) evalues ON evalues.case_id = ecase.case_id' +CHAR(10) + CHAR(10)
	...
	SET @Sql = @Sql + 'FROM (SELECT a.case_id,a.exceptionruleid,a.Flag,a.String,a.Number,a.Date FROM red_dw.dbo.ds_sh_exceptions_values a' + CHAR(10)
	...
	-- SELECT CLAUSE
		SET @Sql = @Sql + 'SELECT cashdr.case_id'
		...
		SET @Sql = @Sql + CHAR(10) + CHAR(9) + ', '' | '') AS ExceptionString'
		 ...
		 SET @Sql = @Sql + CHAR(10) + CHAR(9) + ',FeeEarnerSummary.exstring AS summary_FullExceptionString'
						 + CHAR(10) + CHAR(9) + ',FeeEarnerSummary.tabstring AS summary_TabExceptionString'
						 + CHAR(10) + CHAR(9) + ',FeeEarnerSummary.tab_name AS summary_TabName'
						 + CHAR(10) + CHAR(9) + ',FeeEarnerSummary.tab_order AS summary_TabOrder'
						 + CHAR(10) + CHAR(9) + ',FeeEarnerSummary.MIProjectExceptionTotal AS summary_TabExceptionCount'


		--FROM CLAUSE
		SET @Sql = @Sql + CHAR(10) + 'FROM red_dw.dbo.ds_sh_axxia_cashdr cashdr'
		
		SELECT @Sql = @Sql + CHAR(10) + 'LEFT JOIN #ds_sh_exceptions_values AS [' + CAST(FieldID AS VARCHAR(6)) + '] ON cashdr.case_id = [' + CAST(FieldID AS VARCHAR(6)) + '].case_id AND [' + CAST(FieldID AS VARCHAR(6)) + '].exceptionruleid = ' + CAST(FieldID AS VARCHAR(6)) 
		FROM @FieldList
		GROUP BY FieldID

		SELECT @Sql = @Sql + CHAR(10) + 'LEFT JOIN #FeeEarnerSummary AS FeeEarnerSummary ON cashdr.case_id = FeeEarnerSummary.case_id'
	


		--WHERE CLAUSE
		SET @Sql = @Sql + CHAR(10) + 'WHERE  cashdr.case_id IN (SELECT case_id FROM red_dw.dbo.fact_exceptions_update WHERE datasetid IN (' + @DatasetID + '))
		and cashdr.current_flag = ''Y'''
			 
		--OPTIMIZE CLAUSE
		--SELECT @Sql = @Sql + CHAR(10) + CHAR(10) + 'OPTION (MERGE JOIN, FORCE ORDER, MAXDOP 1)'
		
		--SELECT @Sql = @Sql + CHAR(10) + CHAR(10) + ISNULL(QueryHint, '') FROM Reporting_DW.Exceptions.Datasets WHERE DatasetName = @DatasetName
