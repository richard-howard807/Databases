...
--			now have multiple store procedures.  
--================================================================================================


CREATE PROCEDURE [converge].[cashbook_affinity]
(
    @EndDate AS DATETIME
...
AS 
	...
	IF EXISTS (SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#AffinityCashbook')) 
				DROP TABLE #AffinityCashbook
  ...
  ORDER BY tb.post_date
    ...
    FROM    #AffinityCashbook AS cafintrn WITH ( NOLOCK )
		    LEFT OUTER JOIN axxia01.dbo.cashdr AS cashdr WITH  ( NOLOCK ) ON REPLACE(cashdr.client,' ','') +'-'+REPLACE(cashdr.matter,' ','') = [legacy_client_matter_number]   COLLATE DATABASE_DEFAULT 
