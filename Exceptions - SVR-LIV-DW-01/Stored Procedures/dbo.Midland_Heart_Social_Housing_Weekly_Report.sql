...
SELECT 
	...
	,[TabFilter] = 
	CASE WHEN cboMHInsType.cdDesc IN ('Final Staircasing','Partial Staircasing') THEN 'Staircasing'
		WHEN cboMHInsType.cdDesc IS NULL THEN 'Other'
	ELSE cboMHInsType.cdDesc END
	,CASE WHEN date_closed_case_management IS NULL
	AND completion_date IS NULL THEN 'Live'
	WHEN completion_date IS NOT NULL AND date_closed_case_management IS NULL THEN 'Closed' ELSE 'Exclude' END AS FileStatus
	,revenue.Revenue
