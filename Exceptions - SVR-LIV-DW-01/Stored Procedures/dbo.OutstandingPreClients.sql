...
WHERE matter_number ='00000001') AS MatterOne
 ...
 LEFT OUTER JOIN 
(SELECT dim_matter_header_current.client_code,matter_number,matter_owner_full_name AS LastestMatterOwner
,ROW_NUMBER() OVER (PARTITION BY dim_matter_header_current.client_code ORDER BY TRY_CAST(matter_number AS INT) DESC) AS xOrder
FROM red_dw.dbo.dim_client
INNER JOIN red_dw.dbo.dim_matter_header_current
 ON dim_matter_header_current.client_code = dim_client.client_code
 LEFT OUTER JOIN  
...
AND matter_number<>'ML'
 ...
 ON LatestMatter.client_code = dim_client.client_code AND xOrder=1
