...
CREATE PROCEDURE [dbo].[MI_Exception_Summary_firm_wide_health]
(
...
GROUP BY dim_fed_hierarchy_history.employeeid,date_closed_case_management,dim_matter_header_current.master_client_code
...
SELECT hir.employeeid,
	...
	-- fact_exceptions_update.client_code client, fact_exceptions_update.matter_number matter,
       hir.name,
       hir.fed_code,
       hir.windowsusername,
       ---------------------------------open------------------------------------------------------------
...
-----------------added
    ...
    (
		...
		--WHERE 
WHERE hir.hierarchylevel2hist  IN ( 'Legal Ops - Claims', 'Legal Ops - LTA' )
