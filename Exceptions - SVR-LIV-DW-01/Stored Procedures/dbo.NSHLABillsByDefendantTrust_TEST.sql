...
CREATE PROCEDURE [dbo].[NSHLABillsByDefendantTrust_TEST] --EXEC [dbo].[NSHLABillsByDefendantTrust] '2017-05-01','2018-04-04','Wrightington, Wigan & Leigh NHS Foundation Trust','Liverpool','Clinical Liverpool and Manchester'
(
@StartDate AS DATE
,@EndDate AS DATE
...
/* Added this as a fix to match output on self service*/
,[Fee Total] = MAX(CASE WHEN CAST(dim_bill_date.bill_fin_year-1 AS NVARCHAR(4)) + '/' + CAST(RIGHT(dim_bill_date.bill_fin_year,2) AS NVARCHAR(2)) = '2017/18' THEN dbo.selfservice.[Revenue 2017/2018]						
             WHEN CAST(dim_bill_date.bill_fin_year-1 AS NVARCHAR(4)) + '/' + CAST(RIGHT(dim_bill_date.bill_fin_year,2) AS NVARCHAR(2)) = '2018/19' THEN selfservice.[Revenue 2018/2019]						
             WHEN CAST(dim_bill_date.bill_fin_year-1 AS NVARCHAR(4)) + '/' + CAST(RIGHT(dim_bill_date.bill_fin_year,2) AS NVARCHAR(2)) = '2019/20' THEN selfservice.[Revenue 2019/2020]							
             WHEN CAST(dim_bill_date.bill_fin_year-1 AS NVARCHAR(4)) + '/' + CAST(RIGHT(dim_bill_date.bill_fin_year,2) AS NVARCHAR(2)) = '2020/21' THEN selfservice.[Revenue 2020/2021]						
             WHEN CAST(dim_bill_date.bill_fin_year-1 AS NVARCHAR(4)) + '/' + CAST(RIGHT(dim_bill_date.bill_fin_year,2) AS NVARCHAR(2)) = '2021/22' THEN selfservice.[Revenue 2021/2022]						
         END)
...
LEFT JOIN reporting.dbo.selfservice
ON selfservice.ms_fileid = a.ms_fileid
