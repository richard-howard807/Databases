...
*/
CREATE PROCEDURE [Tableau].[SurreyandSussexPolice]
AS
BEGIN
...
inner join 
       (           --select * from red_dw..fact_bill_matter_detail where client_code  = '00451638' and matter_number = '00000058'                  
              ...
              from red_dw..fact_bill_matter_detail  where  bill_date between DATEADD(month,3,DATEADD(yy, DATEDIFF(yy,1,GETDATE())-1,0)) and DATEADD(month,3,DATEADD(dd,-1,DATEADD(yy, DATEDIFF(yy,0,GETDATE()),0)))
...
left join (	  select   sum(minutes_recorded)/60 as HoursCharged, client_code, matter_number, master_fact_key, client_name --sum(minutes_recorded)/60
			  ...
			  where client_code in  ('00451638','00113147') and dim_transaction_date.transaction_calendar_date  between DATEADD(month,3,DATEADD(yy, DATEDIFF(yy,1,GETDATE())-1,0)) and DATEADD(month,3,DATEADD(dd,-1,DATEADD(yy, DATEDIFF(yy,0,GETDATE()),0))) Group by client_code, matter_number, master_fact_key, client_name  ) tt
