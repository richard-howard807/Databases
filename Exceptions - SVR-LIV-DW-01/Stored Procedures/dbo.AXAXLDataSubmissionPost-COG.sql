...
BEGIN
    ...
    SELECT DISTINCT
           ROW_NUMBER() OVER (PARTITION BY dim_matter_header_current.ms_fileid
                              ORDER BY dim_matter_header_current.ms_fileid
                             ) AS RN,
           dim_matter_header_current.ms_fileid,
           [AXA XL Claim Number] = COALESCE(
                                               assocref.assocRef,
                                               client_reference,
                                               insurerclient_reference,
                                               AXAXLClaimNumber COLLATE DATABASE_DEFAULT,
                                               'TBA'
                                           ),
           [Law Firm Matter Number] = RTRIM(dim_matter_header_current.master_client_code) + '-'
                                      + RTRIM(dim_matter_header_current.master_matter_number),
           ...
           [Product Type New] = CASE
                                    WHEN TRIM(dim_detail_client.[axa_line_of_business]) = 'Motor' THEN
                                        'Motor'
                                    ELSE
                                        ProdType.[CaseText]
                                END,                                                                    -- -- Needs Correcting 20210909 - MT
           [Insured Name] = CASE
                                WHEN ISNULL(dim_detail_claim.[dst_insured_client_name], '') = '' THEN
           ...
           [Is insured Associate a Payor?] = ISNULL(   CASE
                                                           WHEN isPayor.fileID IS NOT NULL THEN
                                                               'Yes'
                                                           ELSE
                                                               'No'
                                                       END,
           ...
           [AXA XL Percentage line share of loss / expenses / recovery] = CASE
                                                                              WHEN udMICoreAXA.pctLineShare > 1 THEN
                                                                                  udMICoreAXA.pctLineShare / 100
                                                                              ELSE
                                                                                  COALESCE(udMICoreAXA.pctLineShare, 1)
                                                                          END,                          -- udMICoreAXA
           ...
           [Law firm handling office (city)] = CASE
                                                   WHEN TRIM(dim_matter_header_current.branch_name) IN ( 'London (Fleet Street)',
                                                                                                         'London (Hallmark)'
                                                                                                       ) THEN
                                                       'London'
                                                   ELSE
           ...
           [Number of Days Since Date Instructions Received/Full] = DATEDIFF(
                                                                                d,
                                                                                COALESCE(
                                                                                            dim_detail_core_details.[date_instructions_received],
                                                                                            dim_matter_header_current.date_opened_case_management
           ...
           [Opposing Side's Solicitor Firm Name] = COALESCE(
                                                               dim_detail_claim.[dst_claimant_solicitor_firm],
                                                               red_dw.dbo.dim_claimant_thirdparty_involvement.claimantsols_name
                                                           ),
           [Reason For instruction] = CASE
                                          WHEN dim_detail_core_details.[proceedings_issued] = 'Yes' THEN
                                              'Litigation'
                                          ELSE
                                              cboReaIns.cdDesc
                                      END,                                                              -- udMICoreAXA
           ...
           [First acknowledgement Date] = COALESCE(
                                                      dim_detail_claim.[axa_first_acknowledgement_date],
                                                      udMICoreAXA.[dteFirstAck]
                                                  ),
        ...
        --                                            ELSE
        --                                                CAST(REPLACE(
        --                                                                RIGHT(dim_detail_concat_cases.[date_subsequent_sla_report_sent], 11),
        --                                                                ']',
        --                                                                ''
        --                                                            ) AS DATE)
        --                                        END,
           ...
           [Date Proceedings Issued] = CASE
                                           WHEN dim_detail_core_details.[proceedings_issued] = 'Yes' THEN
                                               COALESCE(
                                                           ...
                                                           KD_Acknowledgement.[Acknowledgement of Service]
                                                       )
                                       END,
           [AXA XL as defendant] = CASE
                                       WHEN dim_detail_core_details.[proceedings_issued] = 'Yes' THEN
                                           COALESCE(cboIsAXADef.cdDesc, 'Yes')
                                   END,                                                                 -- udMICoreAXA NEW*** 
           [Reason for proceedings] = CASE
                                          WHEN dim_detail_core_details.[proceedings_issued] = 'Yes' THEN
           (CASE
                WHEN cboReForProc.cdDesc = 'Quantum dispute' THEN
                    'Quantum disputes'
                WHEN cboReForProc.cdDesc = 'Insured delay - should be settled' THEN
                    'Insured delay should be settled'
                ELSE
                    cboReForProc.cdDesc
            END
           )
                                      END,                                                              -- udMICoreAXA
           [Proceeding Track] = CASE
                                    WHEN dim_detail_core_details.[proceedings_issued] = 'Yes' THEN
           ...
           [Other Disbursements Paid] = ISNULL(Disbursements.DisbAmount, 0)
                                        - ISNULL(Disbursements.[Disbs - Counsel fees], 0),
           ...
           [Half Life Date] = DATEADD(
                                         ...
                                         DATEDIFF(
                                                     ...
                                                     DateofFinalDisposition.DateofFinalDisposition
                                                 ) / 2,
                                         COALESCE(
                                                     dim_detail_core_details.[date_instructions_received],
                                                     dim_matter_header_current.date_opened_case_management
           ...
           [Date closed] = CASE
                               WHEN fact_finance_summary.unpaid_bill_balance = 0.00
                                    AND dim_detail_core_details.[present_position] IN ( 'Final bill sent - unpaid',
                                                                                        'To be closed/minor balances to be clear'
                                                                                      ) THEN
                                   Receipt.receipt_date
                           END,
           [Date of Final Panel Invoice] = CASE
           ...
           [Claimants Costs Handled by Panel?] = CASE
                                                     WHEN TRIM(cboOutOfIns.cdDesc) IN ( 'Coverage (declined claim)',
                                                                                        'Coverage advice given',
                                                                                        'Discontinued', 'Not pursued',
                                                                                        'Successfully defended (no indemnity payment)',
                                                                                        'Trial (NI won - successful lodgement)',
                                                                                        'Trial (won - no indemnity payment)',
                                                                                        'Trial (won - recovery)'
                                                                                      ) THEN
                                                         'No'
                                                     WHEN TRIM(cboOutOfIns.cdDesc) IN ( 'Settled',
                                                                                        'Settled - Pursuing recovery',
                                                                                        'Trial (Lost)',
                                                                                        'Trial (won - successful part 36 offer)'
                                                                                      ) THEN
                                                         'Yes'
                                                 END,
           ...
           [Outcome of Instruction - Select from list] = CASE
                                                             WHEN cboOutOfIns.cdDesc = 'Discontinued' THEN
                                                                 'Discontinued or not pursued'
                                                             WHEN cboOutOfIns.cdDesc = 'Not pursued' THEN
                                                                 'Discontinued or not pursued'
                                                             WHEN cboOutOfIns.cdDesc = 'Trial (Lost)' THEN
                                                                 'Trial lost'
                                                             WHEN cboOutOfIns.cdDesc = 'Trial (won - recovery)' THEN
                                                                 'Trial won recovery'
                                                             WHEN cboOutOfIns.cdDesc = 'Trial (won - no indemnity payment)' THEN
                                                                 'Trial won no indemnity payment'
                                                             WHEN cboOutOfIns.cdDesc = 'Trial (NI won - successful lodgement)' THEN
                                                                 'Trail north ire won successful lodgement'
                                                             WHEN cboOutOfIns.cdDesc = 'Successfully defended (no indemnity payment)' THEN
                                                                 'Successfully defended no indemnity payment'
                                                             WHEN cboOutOfIns.cdDesc = 'Coverage (declined claim)' THEN
                                                                 'Coverage declined claim'
                                                             ELSE
                                                                 cboOutOfIns.cdDesc
                                                         END,                                           -- udMICoreAXA

           [Was litigation avoidable - Select from list] = CASE
                                                               WHEN cboWasLitAv.cdDesc = 'Yes – other' THEN
                                                                   'Yes other' -- udMICoreAXA
                                                               WHEN cboWasLitAv.cdDesc = 'Yes - other' THEN
                                                                   'Yes other'
                                                               WHEN cboWasLitAv.cdDesc = 'Yes - General delay' THEN
                                                                   'Yes general delay'
                                                               WHEN cboWasLitAv.cdDesc = 'Yes - Insured delay' THEN
                                                                   'Yes insured delay'
                                                               WHEN cboWasLitAv.cdDesc = 'Yes - Insurer delay' THEN
                                                                   'Yes insurer delay'
                                                               WHEN cboWasLitAv.cdDesc = 'Yes – Differing opinions on merits' THEN
                                                                   'Yes differing opinions on merits'
                                                               ELSE
                                                                   cboWasLitAv.cdDesc
                                                           END,
           hierarchylevel3hist,
           ...
           [Last Bill Date] = CASE
                                  WHEN final_bill_flag = 1 THEN
                                      last_bill_date
                              END,
           [Last Time Transaction Date] = last_time_transaction_date,
           [Final Bill Flag] = final_bill_flag,
           Receipt.receipt_date,
           [PanelFeesTest] = ISNULL(fact_finance_summary.disbursements_billed, 0) + ISNULL(defence_costs_billed, 0)
                             + COALESCE(
		   ...
		   dim_detail_claim.[date_90_day_post_instruction_plan_sent] as  [Date 90 day Strategy Plan Sent]
    ...
    FROM red_dw.dbo.dim_matter_header_current WITH (NOLOCK)
        ...
        LEFT JOIN MS_Prod.config.dbAssociates assocref
            ON fileID = ms_fileid
               AND assocType = 'INSURERCLIENT'
        LEFT OUTER JOIN
        (
            SELECT dim_matter_header_current.dim_matter_header_curr_key,
                   MIN(bill_date) AS [Date first invoice sent]
            FROM red_dw.dbo.fact_bill_matter
        ...
        (
            ...
            WHERE client_group_name = 'AXA XL'
                  ...
                  AND
                  (
                      date_closed_case_management IS NULL
                      OR CONVERT(DATE, date_closed_case_management, 103) = '2021-03-29'
                  )
                  AND tskActive = 1
        ...
        (
            ...
            GROUP BY dim_matter_header_current.dim_matter_header_curr_key,
                     ...
                     dim_employee.surname,
                     dim_employee.forename,
        ...
        (
            SELECT dim_matter_header_current.dim_matter_header_curr_key,
                   ...
                   SUM(   CASE
                              WHEN LOWER(cost_type_description) LIKE ('%counsel%') THEN
                                  fact_bill_detail.bill_total_excl_vat
                              ELSE
                                  0
                          END
                      ) AS [Disbs - Counsel fees]
            FROM red_dw.dbo.fact_bill_detail WITH (NOLOCK)
                ...
                LEFT OUTER JOIN red_dw.dbo.dim_bill_cost_type
        ...
        (
            SELECT fileID,
                   MAX(tskDue) AS [Acknowledgement of Service]
            FROM MS_Prod.dbo.dbTasks WITH (NOLOCK)
            WHERE tskActive = 1
                  AND tskType = 'KEYDATE'
                  AND tskDesc = 'Acknowledgement of Service due - today'
            GROUP BY fileID
        ) AS [KD_Acknowledgement]
            ON [KD_Acknowledgement].fileID = dim_matter_header_current.ms_fileid
        LEFT JOIN
        (
            ...
            (
                SELECT COALESCE(
                                   LEFT(Matter.LoadNumber, (CHARINDEX('-', Matter.LoadNumber) - 1)),
                                   Client.AltNumber,
                                   CASE
                                       WHEN ISNUMERIC(Client.Number) = 1 THEN
                                           RIGHT(CAST(CAST(Client.Number AS INT) + 100000000 AS VARCHAR(9)), 8)
                                       ELSE
                                           Client.Number
                                   END
                               ) AS client_code,
                       ISNULL(
                                 RIGHT(Matter.LoadNumber, LEN(Matter.LoadNumber) - CHARINDEX('-', Matter.LoadNumber)),
                                 RIGHT(Matter.AltNumber, LEN(Matter.AltNumber) - CHARINDEX('-', Matter.AltNumber))
                             ) AS matter_number,
                       CASE
                           WHEN LOWER(Payor.DisplayName) LIKE '%axa %' THEN
                               ARDetail.ARTAx
                           ELSE
                               0
                       END AS [AXATax]
            ...
            ON PanelFees.dim_matter_header_curr_key = dim_matter_header_current.dim_matter_header_curr_key
        ...
        (
            SELECT DISTINCT
                   cdCode,
                   cdDesc
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboCovDef'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboCovDef
            ON cboCovDef.cdCode = udMICoreAXA.cboCovDef
...
AND [FirstSubsequentDate].master_matter_number = dim_matter_header_current.master_matter_number
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboReaIns'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboReaIns
            ON cboReaIns.cdCode = udMICoreAXA.cboReaIns
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboIsAXADef'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboIsAXADef
            ON cboIsAXADef.cdCode = udMICoreAXA.cboIsAXADef
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboReForProc'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboReForProc
            ON cboReForProc.cdCode = udMICoreAXA.cboReForProc
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboReaForPanel'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboReaForPanel
            ON cboReaForPanel.cdCode = udMICoreAXA.cboReaForPanel
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboMedOutcome'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboMedOutcome
            ON cboMedOutcome.cdCode = udMICoreAXA.cboMedOutcome
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboOutOfIns'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboOutOfIns
            ON cboOutOfIns.cdCode = udMICoreAXA.cboOutOfIns
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboWasLitAv'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboWasLitAv
            ON cboWasLitAv.cdCode = udMICoreAXA.cboWasLitAv
        ...
        (
            ...
            FROM MS_Prod.dbo.udMapDetail
                JOIN MS_Prod.dbo.dbCodeLookup
                    ON txtLookupCode = cdType
            WHERE txtMSCode = 'cboLineofBus'
                  AND txtMSTable = 'udMICoreAXA'
        ) cboLineofBus
            ON cboLineofBus.cdCode = udMICoreAXA.cboLineofBus
        ...
        (
            SELECT fileID,
                   assocRef AS AXAXLClaimNumber
            FROM MS_Prod.[config].[dbAssociates]
            WHERE 1 = 1
                  AND assocType = 'CLIENT'
        ) AXAXLClaimNumber
            ON CAST(AXAXLClaimNumber.fileID AS NVARCHAR(20)) = CAST(dim_matter_header_current.ms_fileid AS NVARCHAR(20)) COLLATE DATABASE_DEFAULT
        ...
        (
            ...
            ON CAST(Brokername.fileID AS NVARCHAR(20)) = CAST(dim_matter_header_current.ms_fileid AS NVARCHAR(20)) COLLATE DATABASE_DEFAULT
        ...
        (
            ...
            ON isPayor.fileID = dim_matter_header_current.ms_fileid
        LEFT JOIN
        (
            SELECT DISTINCT
                   fileID,
            ...
            ON assoccontname.fileID = dim_matter_header_current.ms_fileid
        ...
        LEFT JOIN SQLAdmin.dbo._20210909_AXA_ProductsandBusinessType LineofBus
            ON LineofBus.[ClNo] = dim_matter_header_current.master_client_code COLLATE DATABASE_DEFAULT
               AND LineofBus.[FileNo] = dim_matter_header_current.master_matter_number COLLATE DATABASE_DEFAULT
               AND LineofBus.[MSCode] = 'cboLineofBus'
        LEFT JOIN SQLAdmin.dbo._20210909_AXA_ProductsandBusinessType ProdType
            ON ProdType.[ClNo] = dim_matter_header_current.master_client_code COLLATE DATABASE_DEFAULT
               AND ProdType.[FileNo] = dim_matter_header_current.master_matter_number COLLATE DATABASE_DEFAULT
               AND ProdType.[MSCode] = 'cboPrrodType'
        ...
        (
            ...
            ON Receipt.dim_matter_header_curr_key = fact_dimension_main.dim_matter_header_curr_key
        ...
        (
            SELECT dim_matter_header_curr_key,
                   (
                       ...
                       ) AS updatedate (LatestDate)
                   ) AS DateofFinalDisposition
            FROM
            (
                SELECT dim_detail_claim.dim_matter_header_curr_key,
                ...
                WHERE CASE
                      ...
                      AND ISNULL(
                                    ...
                                    END,
                                    GETDATE()
                                ) > '2022-06-01' --1 June 2022 

            ) x
        ) DateofFinalDisposition
            ON DateofFinalDisposition.dim_matter_header_curr_key = fact_dimension_main.dim_matter_header_curr_key
    WHERE 1 = 1
          AND dim_matter_header_current.master_client_code = 'A1001'
          ...
          (
              ((CASE
                    ...
                    (
                        ...
                        OR dim_detail_core_details.present_position = 'To be closed/minor balances to be clear'
                    ) THEN
                        ISNULL(last_bill_date, dim_matter_header_current.date_closed_case_management)
                    WHEN
                    (
                        dim_detail_core_details.present_position <> 'Final bill sent - unpaid'
                        AND dim_detail_core_details.present_position <> 'To be closed/minor balances to be clear'
                    ) THEN
                        dim_matter_header_current.date_closed_case_management
                END
               ) >= '2022-06-01'
              ...
              OR red_dw.dbo.dim_matter_header_current.dim_matter_header_curr_key IN
                 (
                     ...
                     WHERE master_client_code = 'A1001'
                           AND date_closed_case_management IS NULL
                           AND ISNULL(present_position, '') NOT IN ( 'Final bill sent - unpaid',
                                                                     'To be closed/minor balances to be clear'
                                                                   )
                 )
          )
          ...
          AND dim_matter_header_current.master_client_code + '-' + dim_matter_header_current.master_matter_number NOT IN ( 'A1001-6044',
                                                                                                 'A1001-10784',
                                                                                                 'A1001-10789',
                                                                                                 'A1001-10798',
                                                                                                 'A1001-10822',
