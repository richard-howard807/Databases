...
-- =============================================
-- ES 06/07/2018 - Changed client group name to look for Co-operative Grouo instead of Co-operative Insurance, added claimant's postcode
-- LD 10/07/2018 - Changed client group name to client group code in the where clause and added 1st Subsequent SLA date Webby 324434
-- ES 10/07/2018 - added fields for dashboard, added claimant solicitor transformation
-- ES 18/07/2018 - removed matters where worktype is 1044-Residential conveyancing (houses/flats), requested by HF
-- ES 24/07/2018 - added NMI1069 - dst_claimant_solicitor_firm to Claimant's solicitor field, requested by HF
...
-- ES 08/09/2022 - #165026 added limitation date and amended co-op handlers
-- =============================================
...
--06/02/2019 - new date parameters
and (date_claim_concluded is null OR date_claim_concluded>='2013-01-01')
AND dim_matter_header_current.date_opened_case_management >='2010-01-01'

AND clients_claims_handler_surname_forename IN ('Byrne, Paul','Cain, Andrew','Gallagher, Saul','Hinchcliffe, Andrea','Leaver, Mark','Stead, Nicola','Webber, Elayne','Wilson, Louise', 'Wild, David','Greene, Marilyn','White, Sally-ann','Pendle, Steph')

--additional logic applied
AND ISNULL(locationidud,'') <> 'Glasgow'
AND work_type_name LIKE 'Motor%'
AND ((CASE WHEN LOWER(work_type_name)='claims handling' THEN COALESCE(fact_detail_reserve_detail.[el_tp_injury_reserve],fact_detail_reserve_detail.[motor_tp_injury_reserve],fact_detail_reserve_detail.[pl_tp_injury_reserve]) ELSE fact_finance_summary.[damages_reserve] END)>=50000
OR (CASE WHEN LOWER(work_type_name)='claims handling' THEN COALESCE(fact_detail_reserve_detail.[el_tp_injury_reserve],fact_detail_reserve_detail.[motor_tp_injury_reserve],fact_detail_reserve_detail.[pl_tp_injury_reserve]) ELSE fact_finance_summary.[damages_reserve] END) IS NULL
)

AND ISNULL(dim_detail_core_details.referral_reason,'') IN ('Dispute on liability and quantum','Dispute on quantum','Dispute on liability')
END
GO
