...
-- Description:	built for Sarah Calvert to show archive Process for files ticket -328257  
-- =============================================
CREATE PROCEDURE [dbo].[MatterSphereArchivingProcessReport]
AS
BEGIN
...
CASE WHEN ISNUMERIC(ds_sh_ms_dbfile.fileno) = 1 THEN RIGHT('00000000' + CONVERT(VARCHAR,ds_sh_ms_dbfile.fileno), 8) ELSE CAST(RTRIM(ds_sh_ms_dbfile.fileno) AS VARCHAR(8)) END) AS VARCHAR(8))
 matter_number,
ds_sh_ms_uddeedwill.created,
createby.usrfullname createby,
dwauther.usrfullname dwauther,
dwdesc,
