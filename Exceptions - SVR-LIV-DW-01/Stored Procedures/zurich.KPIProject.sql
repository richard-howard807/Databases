...
-- added non-zurich motor as a seperate dataset
-- =============================================
--EXEC [zurich].[KPIProject] 'Non-Zurich Motor'
...
SELECT dim_matter_header_current.master_client_code+'-'+dim_matter_header_current.master_matter_number AS [Client/Matter Number]
	...
	, CASE WHEN ISNULL(dim_matter_header_current.client_group_code,'')='00000001' THEN 'Zurich'
		...
		WHEN ISNULL(dim_matter_header_current.client_group_code,'')<>'00000001' AND dim_fed_hierarchy_history.hierarchylevel3hist='Motor' THEN 'Non-Zurich Motor'
	END AS [Client]
...
--88726
AND NOT (dim_fed_hierarchy_history.jobtitle LIKE '%Partner%')
AND NOT (dim_matter_worktype.work_type_group='PL All' AND dim_detail_core_details.track='Multi Track')
AND NOT (dim_matter_worktype.work_type_group='Disease' OR dim_matter_worktype.work_type_group='Motor')
AND NOT (dim_matter_worktype.work_type_name LIKE '%PL - Pol%')
AND NOT (dim_employee.locationidud LIKE '%London%' OR dim_employee.locationidud LIKE'Glasgow')
