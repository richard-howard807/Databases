...
CREATE PROCEDURE [Exceptions].[GetExceptionDatasetCached](
	  @DatasetName NVARCHAR(255)
	, @Debug BIT = 0 -- Returns the sql statement without executing it
	, @Client nvarchar(8) = null
	, @Matter nvarchar(8) = NULL
...
BEGIN
	...
	SET @Sql = @Sql + 'SELECT ds_sh_exceptions_values.* FROM red_dw.dbo.ds_sh_exceptions_values inner join red_dw.dbo.fact_exceptions_update on  ds_sh_exceptions_values.case_id = fact_exceptions_update.case_id and  ds_sh_exceptions_values.exceptionruleid = fact_exceptions_update.exceptionruleid    WHERE ds_sh_exceptions_values.Flag IS NOT NULL)a ) evalues ON evalues.case_id = ecase.case_id' +CHAR(10) + CHAR(10)
	...
	-- SELECT CLAUSE
		...
		BEGIN
		set	
		@Sql = @Sql  + CHAR(10) + 'LEFT JOIN axxia01.dbo.camatgrp feecamatgrp on cashdr.client = feecamatgrp.mg_client and  cashdr.matter = feecamatgrp.mg_matter
		INNER JOIN dbo.split_delimited_to_rows('''+@FeeEarners+''' ,'','') AS FeeEarners ON feecamatgrp.mg_feearn COLLATE database_default = FeeEarners.val COLLATE database_default'
		END
        
		IF @Team IS NOT NULL
		BEGIN
		set	
		@Sql = @Sql  + CHAR(10) + 'LEFT JOIN axxia01.dbo.camatgrp team on cashdr.client = team.mg_client and  cashdr.matter = team.mg_matter
		left join Exceptions.Accounts.Structure structure on  team.mg_feearn= structure.feeearnercode
		INNER JOIN dbo.split_delimited_to_rows('''+@Team+''' ,'','') AS teams ON structure.team COLLATE database_default = teams.val COLLATE database_default'
		END
		...
		begin
		set @Sql = @Sql + CHAR(10) + ' and cashdr.client = ''' +@Client+ ''''
		end

		if @Matter is not null 
		begin 
		set  @Sql = @Sql + CHAR(10) + ' and cashdr.matter = ''' +@Matter+ '''' 
		end
...
END
GO
GRANT EXECUTE ON  [Exceptions].[GetExceptionDatasetCached] TO [SBC\SQL - DataReader on SVR-LIV-DWH-01_Limited]
GO
