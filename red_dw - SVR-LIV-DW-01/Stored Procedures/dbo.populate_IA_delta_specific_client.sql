...
CREATE PROC [dbo].[populate_IA_delta_specific_client] (@dim_client_key int)

AS

-- *******************************************************************************************************************************
-- RH 21/04/2021
-- Main query that sends and updates data to IA is 'populate_IA_delta'

-- This SP can be used when a specific client is missing from IA but is in dim_client_matter_summary & fact_client_matter_summary
...
AND dim_client_matter_summary.dim_client_key = @dim_client_key
...
where crm_client_type in ('Contact MK Comp','Company','Prospect')
...
and (push_to_ia = 0 or push_to_ia is null)
AND dim_client.dim_client_key = @dim_client_key
...
AND cli.dim_client_key = @dim_client_key  
...
WHERE
	ISNULL(phone_number,'') != '' 
and (push_to_ia = 0 or push_to_ia is null)
AND dw2.dim_client_key = @dim_client_key
...
and (push_to_ia = 0 or push_to_ia is null)
AND dw.dim_client_key = @dim_client_key
...
from dbo.dim_client
where client_code NOT like 'EMP%'
and (push_to_ia = 0 or push_to_ia is null)
and dim_client_key = @dim_client_key
...
AND t1.dim_client_key = @dim_client_key 
...
inner join dbo.dim_client on dim_client.client_code = mat.client_code
