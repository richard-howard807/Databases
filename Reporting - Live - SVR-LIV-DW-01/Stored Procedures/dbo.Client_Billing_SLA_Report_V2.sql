...
Description:		This has been rewritten see ticket #105716 
Current Version:	Initial Create
====================================================*/ 

CREATE PROCEDURE [dbo].[Client_Billing_SLA_Report_V2]
...
BEGIN
 ...
 -- DECLARE @Team AS NVARCHAR(MAX) = 'Motor Mainstream'
		--, @FeeEarner AS NVARCHAR(MAX) = 'Andrew Sutton|Sally Veevers'
...
WHERE 
	...
	--AND fact_dimension_main.matter_number = '00000002'
	 dim_all_time_activity.unbilled_record = 1
...
SELECT 
	...
	, CASE 
		...
		('', 'Claim and costs concluded but recovery outstanding', 'Claim and costs outstanding', 'Claim concluded but costs outstanding') 
		THEN 'interim'
		ELSE 'final'
		END	AS [interim_final]
	, CASE 
		WHEN ISNULL(RTRIM(dim_detail_finance.output_wip_fee_arrangement), '') = 'Fixed Fee/Fee Quote/Capped Fee' 
		THEN 'fixed_fee'
		ELSE
		'hourly'
		END	AS [fixed_hourly]
...
INNER JOIN #Team ON #Team.ListValue COLLATE DATABASE_DEFAULT = dim_fed_hierarchy_history.hierarchylevel4hist
...
INNER JOIN #fee_arrangement ON #fee_arrangement.ListValue COLLATE DATABASE_DEFAULT = ISNULL(RTRIM(dim_detail_finance.output_wip_fee_arrangement), 'Missing Data')
...
LEFT OUTER JOIN dbo.client_billing_sla ON client_billing_sla.master_client_code COLLATE DATABASE_DEFAULT = dim_matter_header_current.master_client_code



--LEFT OUTER JOIN (SELECT master_fact_key, SUM(fact_all_time_activity.time_charge_value) AS [internal_counsel_time_value]
...
--				GROUP BY master_fact_key, dim_transaction_date_key
--) AS #internal_counsel ON #internal_counsel.master_fact_key = fact_dimension_main.master_fact_key 
WHERE 
...
FROM (
	...
	WHEN #sla_billing.fixed_hourly = 'hourly' AND #sla_billing.interim_final = 'final' 
		...
		WHEN #sla_billing.fixed_hourly = 'fixed_fee' AND #sla_billing.interim_final = 'final' AND ISNULL(fact_finance_summary.disbursement_balance, 0) >= 30 
		...
		WHEN #sla_billing.fixed_hourly = 'fixed_fee' THEN 
		CASE -- MIB 40% interim bill if claim not settled after 3 months and FF amount is over £150	
		WHEN client_billing_sla.bill_rule_num = 'Rule 5' AND #sla_billing.interim_final = 'interim' AND dim_detail_outcome.date_claim_concluded IS NULL
	...
	WHEN client_billing_sla.bill_rule_num = 'Rule 5' AND #sla_billing.interim_final = 'final' 
		AND ISNULL(fact_finance_summary.defence_costs_billed, 0) < ISNULL(fact_finance_summary.fixed_fee_amount, 0) THEN	
		'fixed fee'
		-- all other FF cases
		WHEN #sla_billing.interim_final = 'final' AND ISNULL(fact_finance_summary.defence_costs_billed, 0) < ISNULL(fact_finance_summary.fixed_fee_amount, 0) THEN	
	'fixed fee'
	ELSE
		'no bill'
	END 
		...
		CASE 
		WHEN #sla_billing.interim_final = 'final' THEN
		'hourly'
		ELSE
		'no bill'
		END
		-- hourly rate interim for clients with different rules for half year/full year to other dates
		WHEN client_billing_sla.bill_rule_num IN ('Rule 10', 'Rule 22') AND #sla_billing.fixed_hourly = 'hourly' AND #sla_billing.interim_final = 'interim'
	...
	WHEN MONTH(GETDATE()) IN (10, 4) AND ISNULL(fact_finance_summary.wip, 0) >= 100 THEN
		...
		END 
		-- hourly rate interim 
		WHEN #sla_billing.fixed_hourly = 'hourly' AND #sla_billing.interim_final = 'interim' AND ISNULL(fact_finance_summary.wip, 0) >= client_billing_sla.wip_minimum 
		...
		WHEN #sla_billing.fixed_hourly = 'hourly' AND #sla_billing.interim_final = 'final' AND ISNULL(fact_finance_summary.wip, 0) >= 50 THEN
		'hourly'
		 ELSE
		'no bill'
		END		AS [bill_type]
