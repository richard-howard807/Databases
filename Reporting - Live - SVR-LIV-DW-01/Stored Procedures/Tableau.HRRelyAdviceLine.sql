...
BEGIN
	...
	, [TimePosted].[Hours Posted]
	, NULL AS [Date of Time Posting]
...
--includes transferred time
LEFT OUTER JOIN (SELECT fact_all_time_activity.dim_orig_matter_header_curr_key, SUM(fact_all_time_activity.minutes_recorded)/60 AS [Hours Posted]
			FROM red_dw.dbo.fact_all_time_activity
			GROUP BY fact_all_time_activity.dim_orig_matter_header_curr_key) AS [TimePosted]
ON [TimePosted].dim_orig_matter_header_curr_key=dim_matter_header_current.dim_matter_header_curr_key
...
UNION
 ...
 SELECT dim_matter_header_current.client_name AS [Client]
	...
	, dim_detail_advice.summary_of_advice AS [Summary of Advice]
	, dim_detail_advice.knowledge_gap AS [Knowledge Gap]
	, SUM(fact_all_time_activity.minutes_recorded)/60 AS [Hours Posted]
	, TransactionDate.calendar_date AS [Date of Time Posting]
	, fact_matter_summary_current.last_time_transaction_date AS [Date of Last Time Posting]
	, 'Hours' AS [Level]
...
ON fact_dimension_main.dim_matter_header_curr_key=fact_all_time_activity.dim_orig_matter_header_curr_key
LEFT OUTER JOIN red_dw.dbo.dim_matter_header_current
ON dim_matter_header_current.dim_matter_header_curr_key = fact_dimension_main.dim_matter_header_curr_key
LEFT OUTER JOIN red_dw.dbo.dim_detail_core_details
ON dim_detail_core_details.dim_detail_core_detail_key = fact_dimension_main.dim_detail_core_detail_key
...
LEFT OUTER JOIN red_dw.dbo.fact_matter_summary_current
ON fact_matter_summary_current.master_fact_key = fact_dimension_main.master_fact_key
LEFT OUTER JOIN red_dw.dbo.dim_date AS [TransactionDate]
ON fact_all_time_activity.dim_transaction_date_key=[TransactionDate].dim_date_key
...
AND dim_matter_worktype.work_type_name = 'Employment Advice Line'
AND (dim_matter_header_current.date_closed_case_management IS NULL OR dim_matter_header_current.date_closed_case_management>= '2019-01-01')
AND TransactionDate.calendar_date>='2019-01-01'

GROUP BY dim_matter_header_current.master_client_code + '-' + dim_matter_header_current.master_matter_number,
         COALESCE(dim_detail_advice.issue, dim_detail_advice.emph_primary_issue, dim_detail_advice.issue_hr),
         COALESCE(dim_detail_advice.secondary_issue, dim_detail_advice.secondary_issue_hr),
         COALESCE(dim_detail_advice.tgif_classification, dim_detail_advice.case_classification),
         COALESCE(
         dim_detail_advice.name_of_caller,
         dim_detail_advice.whitbread_name_of_caller,
         dim_detail_advice.swissport_name_of_caller
         ),
         COALESCE(
         dim_detail_advice.job_title_of_caller_pizza_hut,
         dim_detail_advice.whitbread_caller_job_title,
         dim_detail_advice.job_title_of_caller_emp,
         dim_detail_advice.job_title_of_caller_tgipe
         ),
         COALESCE(dim_detail_advice.region, dim_detail_advice.swissport_region),
