...
Description:		Claims Division MI to drive the Tableau Dashboards, prior years
Current Version:	Initial Create
====================================================
====================================================

*/
CREATE PROCEDURE [Tableau].[SurreyandSussexPolice_PRIOR_YEARS]
AS
BEGIN
...
inner join 
       (           --select * from red_dw..fact_bill_matter_detail where client_code  = '00451638' and matter_number = '00000058'                  
			  ...
			  WHERE  bill_date between DATEADD(month,3,DATEADD(yy, DATEDIFF(yy,1,GETDATE())-3,0)) and DATEADD(month,3,DATEADD(dd,-1,DATEADD(yy, DATEDIFF(yy,0,GETDATE()),0)))
              and  client_code in ( '00451638','00113147') 
              --AND client_code='00451638' AND matter_number='00000190'
              group by 
              client_code,matter_number, dim_matter_header_curr_key, CASE WHEN bill_date BETWEEN '2016-04-01' and '2017-03-31' THEN '2016/17'
					WHEN bill_date BETWEEN '2017-04-01' and '2018-03-31' THEN '2017/18'
					WHEN bill_date BETWEEN '2018-04-01' and '2019-03-31' THEN '2018/19'
					ELSE NULL END
...
LEFT OUTER JOIN (SELECT client_code
				...
				AND transaction_calendar_date BETWEEN DATEADD(month,3,DATEADD(yy, DATEDIFF(yy,1,GETDATE())-3,0)) and DATEADD(month,3,DATEADD(dd,-1,DATEADD(yy, DATEDIFF(yy,0,GETDATE()),0)))
				...
				GROUP BY CASE
						 ...
						 END,
						 client_code,
						 matter_number) AS [Time] ON Time.client_code = fact_dimension_main.client_code
