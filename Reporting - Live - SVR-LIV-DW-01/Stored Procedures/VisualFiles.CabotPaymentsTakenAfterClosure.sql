...
CREATE PROCEDURE [VisualFiles].[CabotPaymentsTakenAfterClosure] 
AS
BEGIN
    SELECT  CDE02ClientAccountNo AS ClientAccountNo ,
            ...
            ,CASE WHEN CRD_DateJudgmentGranted = ''  THEN  NULL ELSE 
             ...
             ,[NoPaymentsReceived]AS [NoPaymentsReceived]
...
INNER JOIN (SELECT AccountInformation.mt_int_code,COUNT(AccountInformation.mt_int_code) AS [NoPaymentsReceived]FROM VFile_Streamlined.dbo.AccountInformation
INNER JOIN VFile_Streamlined.dbo.Payments
 ON AccountInformation.mt_int_code=Payments.mt_int_code
 WHERE ClientName='Marlin'
AND SubClient='Cabot Financial (UK) Limited'
AND PYR_PaymentDate > CLO_ClosedDate
AND MilestoneCode='COMP'
AND CurrentBalance>0
GROUP BY AccountInformation.mt_int_code) AS Payments
 ON AccountInfo.mt_int_code=Payments.mt_int_code
 LEFT
 OUTER JOIN ( SELECT    mt_int_code ,
                                        PostCode
