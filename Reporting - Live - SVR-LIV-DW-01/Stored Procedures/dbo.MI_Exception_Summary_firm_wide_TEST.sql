...
CREATE PROCEDURE [dbo].[MI_Exception_Summary_firm_wide_TEST]
(
...
INNER JOIN red_dw.dbo.dim_employee ON dim_employee.dim_employee_key = dim_fed_hierarchy_history.dim_employee_key  AND dss_current_flag = ''Y'' AND activeud = 1
INNER JOIN Exceptions.dbo.[MI_Management_firm_wide] ON dim_fed_hierarchy_history.employeeid = MI_Management_firm_wide.employeeid AND dss_current_flag = ''Y'' AND activeud = 1
AND (leaverlastworkdate IS NULL OR leaverlastworkdate > @nDate)
WHERE fed_code IN ('+@FeeEarners+')
AND hierarchylevel2hist = ''Legal Ops - Claims'' 
AND LEN(dim_fed_hierarchy_history.employeeid) > 20 
AND ISNUMERIC(fed_code) <> 0
'
