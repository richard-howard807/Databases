...
CREATE PROCEDURE [dbo].[TechnicalChecklistMotor_ChartDates]

--Total completed (number) and something to show a trend – say last 6 months totals to show how use is growing
--Total current % compliance – and a trend to show how the % has changed in the last 6 months
--Total issues identified to date and a trend to show progress over time
...
SELECT 
	   ...
	   /*Completed*/
       [Completed] = SUM(CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Indemnity END +
       CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Nonindemnity END),

	   Indemnity = SUM(CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Indemnity END),

	   Nonindemnity = SUM(CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Nonindemnity END),

	   [Number of matters that should be completed] = SUM(AllData.Number) ,

	   [Number of matters that have not been completed] = SUM(AllData.ScoreISBlank),

	   [Compliance %] = SUM(CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Indemnity END +
       CASE WHEN AllData.ScoreISBlank=1 THEN 0 ELSE AllData.Nonindemnity END)
	   / CAST(SUM(AllData.Number) AS DECIMAL(8,2))
...
, CASE WHEN cboAccPrivLand='Y' THEN 1 ELSE 0 END  + 
 ...
 ,CAST(LEFT(DATENAME(MONTH, date_opened_case_management), 3) AS NVARCHAR(10)) +'-' + CAST(YEAR(date_opened_case_management) AS NVARCHAR(10)) AS MonthYear

 ,MONTH(date_opened_case_management) +YEAR(date_opened_case_management) AS MonthYearSort
