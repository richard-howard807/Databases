...
CREATE PROCEDURE [dbo].[entity_search_report]
(
		@SearchType SMALLINT					
...
--	select * from #entity_search_results
	...
	PRINT CONVERT(VARCHAR(30),GETDATE(),120)
		...
		FROM
				ConflictSearch.dbo.ConflictSearch tb1 (NOLOCK)
			LEFT OUTER JOIN ConflictSearch.dbo.ConflictSearch tb2 (NOLOCK)
			ON  tb1.Reference = tb2.Reference
				AND tb1.SourceID= tb2.SourceID -- Added as we are now pulling data from MS and FED.
				AND tb2.TYPE = 'Address'
				AND tb1.DESCRIPTION LIKE '%Client'
				AND tb1.TYPE = 'Entity'
			INNER JOIN ConflictSearch.dbo.ConflictSearchDrilldownDetailsTable dd (NOLOCK)
			ON  tb1.EntityCode = dd.code
				AND tb1.SourceID= dd.SourceID -- Added as we are now pulling data from MS and FED.
				AND dd.Client NOT IN ( '00121614', '00076202',
										'00047237', '00006930',
				...
				AND dd.Matter NOT IN ('0','ML') -- exclude matter zero and ML
			WHERE
				--( REPLACE(tb2.Postcode,' ','') LIKE '%' + @SearchDetail1 + '%' )
           ( tb2.Postcode LIKE '%' + @SearchDetail1 + '%' )    
                            
		AND (
				tb1.TYPE IN ('Entity','Matter' ) -- only other type is address
			)
		AND ( tb1.[EntityCode] NOT IN ( '00121614', '00076202',
										'00047237', '00006930',
										'00123739', '00030645' ) )
