...
CREATE PROCEDURE [dbo].[ZurichDiseaseNewListingsAdHoc]
...
CASE WHEN ms_only=1 THEN [ClaimantSolicitorNameMS] ELSE [ClaimantSolicitorNameFED] END COLLATE database_default AS claimantsols_name,
CASE WHEN ms_only=1 THEN ClaimantSolRefMS ELSE ClaimantSolRefFED END COLLATE DATABASE_DEFAULT AS claimantsols_reference,
dim_claimant_address.address_line_1,
dim_claimant_address.address_line_2,
dim_claimant_address.address_line_3,
dim_claimant_address.address_line_4,
CASE WHEN ms_only=1 THEN ClaimantSolAddressMS ELSE ClaimantSolAddressFED END  COLLATE DATABASE_DEFAULT AS ClaimantSolicitorAddress,
 CASE
...
,contName AS [ClaimantSolicitorNameMS]
,CONCAT(addLine1,+' '+addLine2,+' '+addLine3,+' '+addLine4,+' '+addLine5,+' '+addPostcode) AS ClaimantSolAddressMS
,addPostcode,assocRef AS ClaimantSolRefMS
 FROM ms_prod.config.dbAssociates
 INNER JOIN MS_Prod.config.dbContact
  ON dbContact.contID = dbAssociates.contID
INNER JOIN ms_prod.dbo.dbAddress
 ON assocdefaultaddID=addID
WHERE assocType='CLAIMANTSOLS') AS MSSOlicitor
 ON ms_fileid=MSSOlicitor.fileID

LEFT OUTER JOIN (SELECT case_id
,cl_clname AS [ClaimantSolicitorNameFED]
,CONCAT(RTRIM (fm_addli1),+' '+RTRIM (fm_addli2),+' '+RTRIM (fm_addli3),+' '+RTRIM (fm_addli4),+' '+RTRIM (fm_poscod)) AS ClaimantSolAddressFED
,reference AS ClaimantSolRefFED
FROM axxia01.dbo.invol
INNER JOIN axxia01.dbo.caclient
 ON entity_code=cl_accode
 INNER JOIN axxia01.dbo.kdclicon
  ON entity_code=kc_client
INNER JOIN axxia01.dbo.fmsaddr
 ON kc_addrid=fmsaddr.fm_addnum

WHERE capacity_code='~ZCLSOLS') AS FEDSolicitor
 ON FEDSolicitor.case_id = dim_matter_header_current.case_id
