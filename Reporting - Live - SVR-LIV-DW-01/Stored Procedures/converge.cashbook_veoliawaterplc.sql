...
CREATE PROCEDURE [converge].[cashbook_veoliawaterplc]
(
    @EndDate AS DATETIME
...
--	DECLARE @EndDate datetime = '20160831'
        
	 
	IF EXISTS (SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#FTBveoliawaterplccashbook')) 
				DROP TABLE #FTBveoliawaterplccashbook
  ...
  INTO #FTBveoliawaterplccashbook				
  FROM [Reporting].[converge].[vw_replicated_trust_balance] tb
     
  WHERE  BankAcctIndex = 43
  	AND tb.gl_date BETWEEN '20090501' AND  @EndDate
  ...
  --SELECT * FROM #FTBCodeveCashbook
    ...
    SELECT  DISTINCT
		   ...
		   CASE 
				 WHEN trust_transaction_type IN ('CLCTR','GEN') THEN 'TRANSFER' -- client transfer
				 ...
				 WHEN trust_transaction_type IN ( 'CLPFC', 'CLCRC','AUTHC','EFT' ) THEN 'PAYMENT' --Client payment from client, returned cheque client account
    ...
    FROM    #FTBveoliawaterplccashbook AS cafintrn WITH ( NOLOCK )
...
--	SELECT DISTINCT trust_transaction_type FROM #FTBveoliawaterplccashbook
