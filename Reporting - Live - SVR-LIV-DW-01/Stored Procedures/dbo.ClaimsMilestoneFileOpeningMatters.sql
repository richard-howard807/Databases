...
,CASE WHEN date_instructions_received IS NOT NULL  AND DATEDIFF(DAY,date_instructions_received,date_opened_case_management)<=0 THEN 1 ELSE 0 END  AS Within1Day
,CASE WHEN date_instructions_received IS NULL THEN 1
WHEN DATEDIFF(DAY,date_instructions_received,date_opened_case_management)>0 THEN 1 ELSE 0 END AS NotWithin1Day
