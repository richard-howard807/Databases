...
--SET @StartDate1='2020-06-01'
--SET @EndDate1='2020-06-30'

--SET @StartDate2='2020-07-01'
--SET @EndDate2='2020-07-31'
------- Start and EndDates----------------------------------------------------------------------
...
WHERE dim_bill_date.bill_date BETWEEN @StartDate1 AND @EndDate1
...
WHERE dim_bill_date.bill_date BETWEEN @StartDate2 AND @EndDate2
GROUP BY  hierarchylevel3hist 
,hierarchylevel4hist
) AS Period1Revenue
...
,AVG(DATEDIFF(DAY,date_opened_case_management,date_claim_concluded)) AS AVGDayToConclude
FROM red_dw.dbo.dim_matter_header_current
INNER JOIN red_dw.dbo.dim_fed_hierarchy_history
 ON fed_code=fee_earner_code COLLATE DATABASE_DEFAULT
...
,AVG(DATEDIFF(DAY,date_opened_case_management,date_costs_settled)) AS AVGDayToSettle
FROM red_dw.dbo.dim_matter_header_current
INNER JOIN red_dw.dbo.dim_fed_hierarchy_history
 ON fed_code=fee_earner_code COLLATE DATABASE_DEFAULT
INNER JOIN red_dw.dbo.dim_detail_outcome
...
WHERE gl_calendar_date BETWEEN @StartDate1 AND @EndDate1
...
LEFT OUTER JOIN red_dw.dbo.fact_budget_activity 
  ON fact_budget_activity.dim_fed_hierarchy_history_key = dim_fed_hierarchy_history.dim_fed_hierarchy_history_key
  AND fact_budget_activity.dim_budget_date_key = fact_agg_billable_time_monthly_rollup.dim_gl_date_key
WHERE gl_calendar_date BETWEEN @StartDate2 AND @EndDate2
AND dim_fed_hierarchy_history.hierarchylevel2hist = 'Legal Ops - Claims'
AND hierarchylevel4hist IS NOT NULL
GROUP BY hierarchylevel3hist,
         hierarchylevel4hist
