...
-- =============================================
CREATE PROCEDURE [dbo].[LLandMTRawData] @Team AS NVARCHAR(200)
	
AS

--TESTING
--DECLARE @Team AS NVARCHAR(50) = 'Large Loss Manchester and Leeds'
...
CASE  WHEN  ( red_dw.dbo.dim_detail_client.coop_master_reporting = 'Yes'OR red_dw.dbo.dim_fed_hierarchy_history.hierarchylevel4hist = 'Fraud and Credit Hire Liverpool'  ) THEN
DATEADD( d,360,
(CASE
...
WHEN department_code = '0027' THEN
ISNULL(
    COALESCE(fact_detail_paid_detail.[personal_injury_paid], fact_detail_client.[zurich_general_damages_psla_only]),   0 ) ELSE
...
--0
--ELSE
--ISNULL(fact_finance_summary.[defence_costs_reserve], 0) - total_amount_billed
--END
--),
--0
--)
...
--)
--AND ISNULL(dim_detail_core_details.[is_this_the_lead_file], 'No') = 'Yes' THEN
--ISNULL(defence_costs_billed, 0) - total_amount_billed
--WHEN
--(
--dim_detail_client.[coop_master_reporting] = 'Yes'
--OR dim_fed_hierarchy_history.hierarchylevel4hist = 'Fraud and Credit Hire Liverpool'
...
--0
--ELSE
--defence_costs_billed - total_amount_billed
--END
--),
--0
...
OR dim_detail_outcome.outcome_of_case LIKE '%disc%' THEN 'Yes' ELSE 'No'
...
--END AS
 dim_detail_outcome.[are_we_pursuing_a_recovery] [Are we Pursuing a Recovery?],
...
fact_finance_summary.total_recovery [Total Recovered],
...
AND ISNULL(dim_detail_core_details.is_this_the_lead_file, 'No') = 'Yes' THEN
ISNULL(defence_costs, 0)  WHEN
(
dim_detail_client.coop_master_reporting = 'Yes'
OR red_dw.dbo.dim_fed_hierarchy_history.hierarchylevel4hist = 'Fraud and Credit Hire Liverpool'
) THEN 0 ELSE defence_costs_billed END AS [Profit Costs Billed To Date (net of VAT)],
vat_amount [VAT],
...
ISNULL(red_dw.dbo.fact_finance_summary.wip, 0) [Unbilled WIP],
...
,[instruction_type]  = CASE WHEN dim_matter_header_current.master_client_code + '-'+ dim_matter_header_current.master_matter_number
IN ('W24438-1', 'W24438-40','W24438-41', 'W24438-42' ) 
...
/* Temp as waiting on fields into DWH */
LEFT JOIN (
  SELECT DISTINCT [fileID], dbCodeLookup.cdDesc AS msg_instruction_type  FROM [MS_Prod].[dbo].[udMIClientMSG]
  LEFT JOIN [MS_Prod].[dbo].dbCodeLookup ON  cdCode = [cboInsTypeMSG]  AND cdType = 'MSGINSTYPE' 

  WHERE cdDesc IS NOT NULL 
) ms ON ms.fileID = ms_fileid
...
AND curclacostrecur IS NOT NULL
WHERE 1 =1 
--AND (dim_client.client_group_code = '00000004'OR dim_client.client_code = 'W24438')
AND dim_client.client_code IN  ('00046018', 'C15332', 'C1001', 'W24438')
AND department_code <> '0027'
