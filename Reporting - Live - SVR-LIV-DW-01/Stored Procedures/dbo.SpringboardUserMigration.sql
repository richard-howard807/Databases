...
,role_responsibility_1,role_responsibility_2
FROM red_dw.dbo.dim_fed_hierarchy_history
LEFT OUTER JOIN MS_Prod.dbo.dbUser
 ON fed_code=usrAlias collate database_default
LEFT OUTER JOIN (SELECT DISTINCT dim_employee_key,role_responsibility_1,role_responsibility_2 FROM red_dw.dbo.dim_employee WHERE ISNULL(role_responsibility_1,'')<>'' OR ISNULL(role_responsibility_2,'')<>'' )  AS Roles
 ON dim_fed_hierarchy_history.dim_employee_key=Roles.dim_employee_key
