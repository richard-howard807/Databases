...
-- Description:	<report to drive the new inter europe report, copy of SabreListing1 38640>
-- =============================================
...
CREATE PROCEDURE [dbo].[InterEuropeListingReport]
AS
BEGIN
	...
	RTRIM(fact_dimension_main.client_code) + '/' + fact_dimension_main.matter_number AS [Weightmans Reference],
		   ...
		   dim_client_involvement.insurerclient_name AS [Insurer Client],
           ...
           END AS [Track],
           CASE
               WHEN dim_detail_core_details.motor_status = 'Cancelled            ' THEN
                   'Closed'
               ELSE
                   'Open'
           END AS [Status],
...
AND 
           ...
           CASE
               WHEN ISNULL(dim_detail_core_details.referral_reason, '') = 'Nomination only                                             ' THEN
                   1
               ELSE
                   0
    ...
    WHERE dim_client.client_code = '00351402'
