...
DROP TABLE IF EXISTS #Disbursements
		...
		FROM (
						...
						SELECT client_code, matter_number, bill_cal_year, SUM(bill_total_excl_vat) Disbursements
	...
	,#VatBilled.[2021] AS VatBilled_2021
	...
	LEFT JOIN #VatBilled ON #VatBilled.client_code = dim_matter_header_current.client_code AND #VatBilled.matter_number = dim_matter_header_current.matter_number	
	
	WHERE #Revenue.[2021] IS NOT NULL 
	OR #Disbursements.[2021] IS NOT NULL OR #VatBilled.[2021] IS NOT NULL 
GO
