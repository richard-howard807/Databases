...
CREATE PROC [dbo].[AllDebtDump_BK20170929] AS 
...
LEFT JOIN TE_3E_Prod.dbo.Matter ON Matter.MattIndex = InvMaster.LeadMatter
...
LEFT JOIN TE_3E_Prod.dbo.Client ON Client.ClientIndex = Matter.Client

LEFT JOIN TE_3E_Prod.dbo.Payor ON Payor.PayorIndex = InvPayor.Payor
...
LEFT JOIN red_dw.dbo.dim_client AS master_client 
				ON CASE WHEN ISNUMERIC(dim_matter_header_current.master_client_code) = 1 THEN RIGHT('00000000' + CONVERT(VARCHAR,dim_matter_header_current.master_client_code), 8) ELSE CAST(RTRIM(dim_matter_header_current.master_client_code)  AS VARCHAR(8)) END
