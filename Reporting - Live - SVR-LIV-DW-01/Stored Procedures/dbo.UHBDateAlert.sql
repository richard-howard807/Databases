...
CREATE PROCEDURE [dbo].[UHBDateAlert]
AS
BEGIN
SET NOCOUNT ON
...
FROM 
(
SELECT CASE WHEN @test=1 THEN 'Kevin.Hansen@Weightmans.com' ELSE 'Jen.Britton@weightmans.com' END  AS [To]
,'UHB ' + ISNULL([Email Type],'') + ' Key Date Alert' AS [Subject]
,PRO1357 AS [Date Alert]
,'<font size="2" face= "Lucida Sans Unicode">Dear All, <p> The Term End Date ' +  Convert(varchar,PRO1357,103) +
' for ' + ISNULL(RTRIM(PRO1953),'') + ',' + ISNULL(RTRIM(PRO1351),'') + ' is coming up in ' +  ISNULL([Email Type],'') + '<p>' +
CASE WHEN ISNULL([Email Type],'') IN ('3 Month','2 Month') THEN 
'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary, otherwise you will receive another reminder in 1 month'
WHEN ISNULL([Email Type],'') ='1 Month' THEN 'If this needs to be updated or is incorrect please email ' + 'UHB.Propertyview@Weightmans.com'  + ' and we will update where necessary, otherwise you will receive another reminder in 3 weeks'
WHEN ISNULL([Email Type],'')='1 Week' THEN 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.'
ELSE 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.' END + '<p>' + 'Kindest regards' + '<p>' + 'Weightmans' AS Body
FROM 
(
...
,CAST(txtBuildingName AS VARCHAR(MAX)) collate database_default AS PRO1953
...
LEFT OUTER JOIN (SELECT fileID,txtBuildingName FROM MS_Prod.dbo.udMIPropertyView) AS PRO1953
 ON ms_fileid=PRO1953.fileID
WHERE  dim_matter_header_current.client_code='00516358'
...
SELECT CASE WHEN @test=1 THEN 'emily.smith@Weightmans.com' ELSE 'Jen.Britton@weightmans.com' END  AS [To]
,'UHB ' + ISNULL([Email Type],'') + 'Key Date Alert' AS [Subject]
,PRO1594 AS [Date Alert]
,'<font size="2" face= "Lucida Sans Unicode">Dear All, <p> The Tenant Break Date for ' +  ISNULL(RTRIM(PRO1953),'') +
', ' + ISNULL(RTRIM(PRO1351),'') + ' is ' + Convert(varchar,PRO1594,103) +'.' + '<p>'+ 'This is coming up in ' +  ISNULL([Email Type],'') + ' and this is the latest date by which the notice can be served (you should allow an additional 3 days service).' + '<p>' +
CASE WHEN ISNULL([Email Type],'') IN ('3 Month','2 Month') THEN 
'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary, otherwise you will receive another reminder in 1 month'
WHEN ISNULL([Email Type],'') ='1 Month' THEN 'If this needs to be updated or is incorrect please email '  + 'UHB.Propertyview@Weightmans.com' + ' and we will update where necessary, otherwise you will receive another reminder in 3 weeks'
WHEN ISNULL([Email Type],'')='1 Week' THEN 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.'
ELSE 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.' END + '<p>' + 'Kind regards' + '<p>' + 'Weightmans'
FROM 
(
...
,CASE WHEN DATEDIFF(DAY,@StartDate,dim_detail_property.[actual_break_date])=7 THEN '1 Week'
 WHEN DATEADD(MONTH,-1,dim_detail_property.[actual_break_date]) = CAST(@StartDate AS date) THEN '1 Month'
 WHEN DATEADD(MONTH,-2,dim_detail_property.[actual_break_date]) = CAST(@StartDate AS date) THEN '2 Month'
 WHEN DATEADD(MONTH,-3,dim_detail_property.[actual_break_date]) = CAST(@StartDate AS date) THEN '3 Month'
 END AS [Email Type]
,dim_detail_property.[address] AS PRO1351
,CAST(txtBuildingName AS VARCHAR(MAX)) collate database_default AS PRO1953
FROM red_dw.dbo.dim_matter_header_current
...
WHEN ISNULL([Email Type],'') ='1 Month' THEN 'If this needs to be updated or is incorrect please email ' + 'UHB.Propertyview@Weightmans.com'  + ' and we will update where necessary, otherwise you will receive another reminder in 3 weeks'
WHEN ISNULL([Email Type],'')='1 Week' THEN 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.'
ELSE 'If this needs to be updated or is incorrect please email UHB.Propertyview@Weightmans.com and we will update where necessary.' END + '<p>' + 'Kindest regards' + '<p>' + 'Weightmans'
FROM 
(
