...
-- Date:		20160812
-- Description:	Listing of all transactions on ST ONLY the escrow
--========================================================

   Version History
**********************************************************
   Version	 By	    Date		Detail
   1.1		 JL		09-09-2016	Due to poor performance issues we have created a 
								table to hold all transactions and append only the current months data
   1.2		 JL		04-10-2016	Added AUTHE to payment within Category field 
   
 ******************************************
  Testing
-- exec [converge].[st_cashbook_build] '20161031'
*/
CREATE PROCEDURE [converge].[st_cashbook_build] 
(
...
--	DECLARE @EndDate AS DATE = '20161109'


	IF EXISTS (SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#SevernTrentCashbook')) 
				DROP TABLE #SevernTrentCashbook
	...
	SELECT	
			...
			,IsReversed		-- LD 1.3
			,IsDeposit		-- LD 1.3	
			,CASE WHEN tb.postsource = 'TRSTTRSF'  AND transfer_detail_cash_journal IS NULL  AND gl_date > '20160930' THEN 1 ELSE 0 END [Exclude] 

	INTO #SevernTrentCashbook			
	FROM [converge].[vw_replicated_trust_balance] tb
	WHERE gl_date BETWEEN '20111001' AND  @EndDate 
			AND BankAcctIndex  = 71
			

	
	TRUNCATE TABLE converge.cashbook_severntrent

	INSERT INTO converge.cashbook_severntrent

	--------go get FED data and join to 3E data-----
    SELECT  CASE 
            ...
            (CASE WHEN payments.PaymentVAT IS NULL AND payments.PaymentNet IS NOT NULL THEN 0 
			       ELSE payments.PaymentVAT * -1 
				     END) AS PaymentVAT,
			...
			,CASE WHEN amount <0 THEN amount *-1 
			 ...
			 (CASE WHEN LEN(PaymentNotes) > 0 THEN PaymentNotes
				   WHEN LEN(RecoveryNotes) > 0 THEN RecoveryNotes
				    ELSE cafintrn.[narrative] COLLATE DATABASE_DEFAULT 
					 END) AS [Payment Notes],
		...
		,  cafintrn.gl_date
		,  cafintrn.tran_date AS transaction_date
		,  cafintrn.sequence_number [3e_sequence_number]
		,	cafintrn.amount [3e_amount]
		,  GETDATE() AS InsertDate /*1.1 JL*/

	
		                                  
    FROM    #SevernTrentCashbook AS cafintrn 
		    INNER JOIN axxia01.dbo.cashdr AS cashdr WITH ( NOLOCK ) ON cafintrn.client = REPLACE(cashdr.client,' ','') COLLATE DATABASE_DEFAULT
			AND cafintrn.matter = REPLACE(cashdr.matter,' ','') COLLATE DATABASE_DEFAULT
            ...
            LEFT OUTER JOIN ( SELECT    VE00167.case_id,
					...
					OUTER APPLY (	SELECT SUM(VE00160.case_value) NewTotalPaid_Total
									...
									LEFT JOIN #SevernTrentCashbook AS caftotal 
