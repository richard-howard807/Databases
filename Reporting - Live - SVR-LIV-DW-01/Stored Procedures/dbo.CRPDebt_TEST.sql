...
CREATE PROCEDURE [dbo].[CRPDebt_TEST]
(
...
SELECT dim_client.client_name AS [Client Name],
       ...
       ISNULL(matter_partner_code, 'Unknown') AS client_partner_code,
       ISNULL(matter_partner_full_name, 'Unknown') AS [Client Partner],
...
FROM red_dw.dbo.fact_payor_debt_detail fact_debt
    ...
    INNER JOIN #Partner AS [Partner]
        ON [Partner].ListValue COLLATE DATABASE_DEFAULT = ISNULL(matter_partner_code, 'Unknown') COLLATE DATABASE_DEFAULT
