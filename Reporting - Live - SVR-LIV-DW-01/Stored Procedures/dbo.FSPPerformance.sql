...
(
    ...
    FROM dbo.FSPHoursTargets2223 WITH(NOLOCK)
) AS cp
UNPIVOT 
(
...
(
    SELECT [Cascade No], May, June,July,
           Aug, Sept, Oct,Nov,Dec,Jan,Feb,Mar,Apr
    FROM dbo.FSPRevenueTargets2223 WITH(NOLOCK)
) AS cp
UNPIVOT 
(
...
(
    SELECT [Cascade No], May, June,July,
           Aug, Sept, Oct,Nov,Dec,Jan,Feb,Mar,Apr
    FROM dbo.FSPUtilisationTargets2223 WITH(NOLOCK)
) AS cp
UNPIVOT 
(
...
IF OBJECT_ID(N'tempdb..#OverdueTasks') IS NOT NULL
BEGIN
DROP TABLE #OverdueTasks
END

SELECT  usrAlias,COUNT(1) AS NumberOverDueTask 
INTO #OverdueTasks
FROM MS_Prod.dbo.dbTasks WITH(NOLOCK)
INNER JOIN red_dw.dbo.dim_matter_header_current WITH(NOLOCK)
 ON fileID=ms_fileid
INNER JOIN MS_Prod.dbo.dbUser WITH(NOLOCK)
 ON dbTasks.feeusrID=dbUser.usrID
...
LEFT OUTER JOIN #OverdueTasks
 ON FixedSharePartners.fed_code=#OverdueTasks.usrAlias COLLATE DATABASE_DEFAULT
LEFT OUTER JOIN 
(
...
FROM Visualisation.dbo.ROI WITH(NOLOCK)
...
FROM red_dw.dbo.fact_agg_kpi_monthly_rollup WITH(NOLOCK)
INNER JOIN red_dw.dbo.dim_gl_date WITH(NOLOCK)
...
FROM red_dw.dbo.fact_debt_daily WITH(NOLOCK)
INNER JOIN red_dw.dbo.dim_fed_hierarchy_history WITH(NOLOCK)
