...
-- =============================================
	...
	CREATE PROCEDURE [VisualFiles].[MarlinTransactionReport]
	...
	SET @VEndDate = @EndDate   
		...
		BEGIN
			 ...
			 LEFT OUTER JOIN (SELECT     TOP 100 PERCENT mt_int_code
			   ...
			   ON  AccountInfo.[mt_int_code] = NewCurrentBalance.mt_int_code      
				...
				LEFT OUTER JOIN  (SELECT  Payments.mt_int_code ,
								 ...
								 FROM  [SVR-LIV-VISF-01].[Vfile_Live].[dbo].[payment]    AS Payments 
								 INNER JOIN VFile_Streamlined.dbo.AccountInformation ON AccountInformation.mt_int_code = Payments.mt_int_code
			...
			WHERE   ClientName =@Clientname
					...
					AND  LOWER(LastPayment.PaymentTakenByClient) =  'no'
