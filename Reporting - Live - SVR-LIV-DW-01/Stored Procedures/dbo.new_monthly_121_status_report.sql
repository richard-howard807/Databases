...
--========================================================================================================================
-- Healthcare May 20 data due to them using both forms in this month
--========================================================================================================================
	...
	FROM (
			SELECT red_dw.dbo.dim_fed_hierarchy_history.name [employee_name]
					,red_dw.dbo.dim_employee.employeeid
					,CASE 
						WHEN Excluded.ExcludeCount >= 0.5 THEN 
							0
						WHEN NewOneToOne.Status = 'Completed' THEN
							1
						WHEN OldOneToOne.meeting_count = 1 THEN
							1
						ELSE 
							0							 
					  END					AS OneToOneFlag
			...
			FROM red_dw.dbo.dim_employee
				...
				LEFT JOIN (
					...
					ON red_dw.dbo.dim_employee.employeeid= OldOneToOne.employeeid AND OldOneToOne.RowNo=1
				LEFT OUTER JOIN (
									SELECT 
										Detail.UserId
										, 1							AS [meeting_count]
					...
					ON dim_fed_hierarchy_history.windowsusername COLLATE DATABASE_DEFAULT = NewOneToOne.UserId COLLATE DATABASE_DEFAULT AND NewOneToOne.row_no = 1
	...
	WHERE 
		AllData.employeestartdate NOT BETWEEN @StartDate AND @EndDate
		AND @Division = 'Legal Ops - Claims' 
		AND AllData.department = 'Healthcare' 
		AND @CalendarMonth = '202005' 
	ORDER BY 
		AllData.employee_name
