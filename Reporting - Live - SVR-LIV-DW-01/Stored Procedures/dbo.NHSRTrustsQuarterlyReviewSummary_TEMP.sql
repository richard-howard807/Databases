...
-- =============================================
CREATE PROCEDURE [dbo].[NHSRTrustsQuarterlyReviewSummary_TEMP]
(
...
DECLARE @last_year AS DATE = '2021-04-01' --DATEADD(MONTH, -11, GETDATE()+1)-DAY(GETDATE())
DECLARE @current_date AS DATE = '2021-09-30'
...
SELECT 
	...
	, CASE
		--non-clinical
		WHEN RTRIM(dim_detail_health.nhs_scheme) IN ('DH Liab', 'PES', 'LTPS') AND dim_detail_outcome.date_claim_concluded BETWEEN @last_year AND @current_date THEN 
			1
		ELSE
			0
	  END							AS [Non-Clinical]
	, CASE
		...
		AND dim_detail_outcome.date_claim_concluded BETWEEN @last_year AND @current_date THEN
			1
		ELSE	
			0
	  END							AS [Clinical]
...
WHERE
	...
	AND dim_detail_outcome.date_claim_concluded <= @current_date
...
WHERE
	dim_date.calendar_date BETWEEN @last_year AND @current_date
