...
SELECT 
	...
	, CASE WHEN TRIM([output_wip_fee_arrangement]) <>  'Fixed Fee/Fee Quote/Capped Fee'
	 THEN fact_finance_summary.[revenue_estimate_net_of_vat] ELSE
	      fact_detail_cost_budgeting.fees_estimate	END			AS [Fees Estimate - Fees Estimate]
	...
	, Doogal.Latitude
	, Doogal.Longitude
	,TRIM([output_wip_fee_arrangement]) AS [Fee Arrangement]
...
FROM red_dw.dbo.dim_matter_header_current
	...
	LEFT JOIN red_dw.dbo.dim_detail_finance
		ON dim_detail_finance.dim_matter_header_curr_key = dim_detail_outcome.dim_matter_header_curr_key
WHERE 1 =1 
