...
CREATE PROCEDURE [dbo].[MotorSLAC_BK20190724v2]
(
	@Team AS VARCHAR(MAX) 
	...
	,@ClosedCases AS VARCHAR (10)
)
AS
BEGIN
	--For testing purposes
	--===========================================================
	--DECLARE  @Team AS VARCHAR(MAX) = 'Motor Liverpool and Birmingham'
	--, @FedCode AS VARCHAR(MAX) = '1363,5651'
	--,@StartDate AS DATE = '2013-01-01'
	--,@EndDate AS DATE = '2019-07-24'
	--,@Status AS VARCHAR(MAX)  = 'Open,Re-opened,Closed'
	--,@PresentPosition AS VARCHAR(MAX) = 'Claim and costs outstanding,To be closed/minor balances to be clear'
	--,@Client AS VARCHAR(MAX)  = 'A3003'
	--,@ClosedCases AS VARCHAR (10) = 'Open'
...
WHERE CASE WHEN @ClosedCases = 'Open' AND date_closed_case_management is null THEN 1 
		   WHEN @ClosedCases = 'Closed' and date_closed_case_management is not null then 1
					 ELSE 0	
				END = 1
