...
-- Description:	Data submission version of ZurichDiseaseNewListings2019
-- =============================================

CREATE PROCEDURE [dbo].[ZurichDiseaseNewListingsSubmission]
...
SELECT DISTINCT  
	...
	case
	when  isnull(WPS344,'') <> ''  then UPPER(WPS344)  
	...
	dim_matter_header_current.master_client_code + '.' + dim_matter_header_current.master_matter_number		AS [Panel Reference],
	CAST(dim_detail_core_details.[date_instructions_received] AS DATE) AS [Date Received],
	...
	upper(rtrim(dim_detail_claim.[location_of_claimants_workplace])) as [Work Location of Claimant (Town)],
	WPS277 as [Current Reserve],
	...
	dim_detail_litigation.are_weightmans_on_the_court_record  AS [Litigated (Lead Only)],
	CAST(isnull(WPS386,date_settlement_form_sent_to_zurich) AS DATE)		 AS [Closure Date],
	upper(rtrim(WPS276)) as [Lead/Follow],
	cast(WPS283 as varchar(250)) + '%' as [Our Proportion % of Damages],
	...
	CASE
		...
		ELSE
			'Transparent'
	END				AS current_reserve_colour,
	CASE 
		WHEN WPS387 is not null THEN  
			RTRIM(WPS387)
	...
	END				AS [claim_status],
	ClaimDetails.catalina_claim_number
...
where 
	...
	AND ISNULL(CAST(isnull(WPS386,date_settlement_form_sent_to_zurich) AS DATE), '9999-12-31') >= '2022-01-01'
	AND ISNULL(CASE 
			WHEN WPS387 is not null THEN  
				RTRIM(WPS387)
			WHEN dim_detail_critical_mi.[claim_status] is null then
			...
			ELSE
				RTRIM(dim_detail_critical_mi.[claim_status])
		END, '') <> 'Cancelled'
	AND rtrim(WPS332) <> 'MMI'
	AND NULLIF(ClaimDetails.catalina_claim_number, '') IS NULL
ORDER BY	
	[Date Received]
	;
